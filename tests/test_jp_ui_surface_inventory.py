...
class JapaneseUiSurfaceInventoryTests(unittest.TestCase):
    ...
    def test_stage_one_keeps_explicit_unknowns(self):
        ...
        self.assertIn(
            "exact ownership of low-signal byte sequences outside the 256 reviewed "
            "system/graphics/ending/scenario/text/UI-bank candidates: the one/two-"
            "signal scan region-classifies 6,612 candidates, proves 255 rows are "
            "word/graphics/layout data, and identifies retained scenario-level "
            "prefix L-; base-relative, indexed, and dynamic access elsewhere "
            "remains possible",
            self.result["remaining_inventory_gaps"],
        )
