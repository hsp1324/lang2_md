...
OUTPUT_FIELDS = {
    ...
    "ui_patch_surfaces.json": ("target_korean", 128),
}
...
class TranslationTargetResidueTests(unittest.TestCase):
    ...
    def test_middle_scenario_descriptions_preserve_japanese_source_events(self):
        ...
        self.assertEqual(set(builder.TRAILING_SCENARIO_DESCRIPTION_GLYPH_TEXT), {"롤", "렉", "회"})
        ...
        self.assertIn("대륙 대부분을 정복한 베른하르트", scenario_texts[15])
        self.assertNotIn("대륙 절반을", scenario_texts[15])
        self.assertIn("정예부대를 뚫고 온 일행", scenario_texts[16])
        self.assertIn("그가 바로 황제 베른하르트였다.", scenario_texts[16])
        self.assertIn("그 곁에 남자가 숨어 있었다.", scenario_texts[16])
        self.assertIn("사악한 미소를 띤 그는 일행을", scenario_texts[16])
        self.assertNotIn("황제 베른하르트.\n", scenario_texts[16])
        self.assertIn("마물이 습격한 마을", scenario_texts[17])
        self.assertIn("마물을 조종하던 이는 리아나였다.", scenario_texts[17])
        self.assertNotIn("조종하고 있던 사람", scenario_texts[17])
        ...
        self.assertIn("검 한 자루가 그 앞 제단에", scenario_texts[21])
        self.assertIn("보젤이 외는 주문은", scenario_texts[21])
        self.assertNotIn("그의\n입에서 흘러나온 주문", scenario_texts[21])
        self.assertIn("기회를 노리던 베른하르트", scenario_texts[22])
        self.assertIn("엘프의 유적으로 향했다.", scenario_texts[22])
        self.assertNotIn("죽은 줄 알았던", scenario_texts[22])
        self.assertNotIn("엘라드의 유적", scenario_texts[22])
        self.assertNotIn("회", builder.scenario_description_glyph_text(scenario_texts[22]))
        self.assertEqual(builder.RETIRED_SCENARIO23_DESCRIPTION_GLYPH_COMPATIBILITY_TEXT, "죽")
        joined = "\n".join(scenario_texts)
