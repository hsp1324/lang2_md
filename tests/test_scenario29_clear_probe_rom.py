...
class Scenario29ClearProbeTests(unittest.TestCase):
        ...
        *,
        ...
        if completion_target_only:
            target = (
                ...
                + probe_builder.FOLGER_RECORD_INDEX * FIXED_RECORD_SIZE
            )
            allowed.update(
                {
                    target + FIELD_OFFSETS["x"],
                    target + FIELD_OFFSETS["y"],
                }
            )
    ...
    def test_completion_target_moves_only_folger_coordinate(self):
        ...
        for index in range(layout.record_count):
            ...
            if index == probe_builder.FOLGER_RECORD_INDEX:
                self.assertEqual(
                    coordinates,
    ...
    def test_completion_wrapper_targets_only_runtime_folger(self):
    ...
    def test_checksum_is_current_and_valid(self):
        ...
        self.assertEqual(expected, 0x53B6)
