...
class Scenario16ClearProbeTests(unittest.TestCase):
    ...
    def test_source_locks_scheduled_table_handlers_and_text_pointers(self):
        ...
        for target_turn, addresses in probe_builder.TURN_EVENT_TEXTS.items():
            handler = (
                "turn-1-entry" if target_turn == 1 else "turn-8-end"
            )
    ...
    def test_turn_event_probe_rejects_changed_source_surfaces(self):
        cases = (
            ...
            (
                probe_builder.TURN_EVENT_HANDLER_RANGES["turn-3-end"][0],
                "turn handler turn-3-end",
            ),
            (
                probe_builder.TURN_EVENT_HANDLER_RANGES["turn-8-end"][0],
                "turn handler turn-8-end",
            ),
        )
    ...
    def test_default_and_completion_checksums_are_locked(self):
        ...
        for target_turn, checksum in ((3, 0x9307), (8, 0x9311)):
            with self.subTest(target_turn=target_turn):
                data = bytearray(self.production)
                self.assertEqual(
                    probe_builder.patch_probe(
