...
from tools import build_scenario30_clear_probe_rom as probe_builder
from tools.scenario_data import FIELD_OFFSETS, FIXED_RECORD_SIZE, scenario_layout


class Scenario30ClearProbeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_preserves_great_dragon_and_distinct_witch_records(self):
        data = self.patched()
        layout = scenario_layout(self.source, probe_builder.SCENARIO_NUMBER)
        expected = {
            ...
            probe_builder.VISIBLE_WITCH_RECORD_INDEX: (
                0x6E,
                0x3F,
                False,
                1,
            ...
            probe_builder.HIDDEN_WITCH_RECORD_INDEX: (
                0x6F,
                0x48,
                True,
                7,
        ...
        for index, values in expected.items():
            name_id, class_id, hidden, level, coordinates = values
            base = layout.records_offset + index * FIXED_RECORD_SIZE
    ...
    def test_checksum_is_current_and_valid(self):
        ...
        self.assertEqual(expected, 0x68C8)
