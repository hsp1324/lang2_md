...
class Scenario17ClearProbeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_completion_layout_moves_only_elwin_below_source_bernhardt(self):
    ...
    def test_protagonist_death_event_is_locked(self):
        ...
        end = start + len(probe_builder.PROTAGONIST_DEATH_EVENT_BYTES)
        self.assertEqual(
            self.source[start:end],
            probe_builder.PROTAGONIST_DEATH_EVENT_BYTES,
        )
        self.assertEqual(
            int.from_bytes(self.source[start + 4 : start + 8], "big"),
            probe_builder.PROTAGONIST_DEATH_TEXT,
        )
        self.assertEqual(self.source[start + 8 : start + 10], b"\x13\xFF")
    ...
    def test_preserves_bosses_and_hidden_reinforcements(self):
        ...
        layout = scenario_layout(self.source, probe_builder.SCENARIO_NUMBER)
        expected = {
            probe_builder.BERNHARDT_RECORD_INDEX: (0x0E, 0x4E, 1, 52, 37, False),
            probe_builder.BOZEL_RECORD_INDEX: (0x10, 0x61, 1, 38, 29, False),
            9: (0x31, 0x38, 10, 36, 27, True),
            10: (0x32, 0x38, 10, 36, 27, True),
        }
        ...
        bernhardt = layout.records_offset + (
            probe_builder.BERNHARDT_RECORD_INDEX * FIXED_RECORD_SIZE
        )
        self.assertEqual(
            (
                self.source[bernhardt + FIELD_OFFSETS["x"]],
                self.source[bernhardt + FIELD_OFFSETS["y"]],
            ),
            probe_builder.BERNHARDT_POSITION,
        )
