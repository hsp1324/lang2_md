...
class Scenario26ClearProbeTests(unittest.TestCase):
    ...
    def test_protagonist_death_trigger_and_event_are_locked(self):
        ...
        self.assertEqual(
            int.from_bytes(
                self.source[trigger_start + 4 : trigger_start + 8],
                "big",
            ),
        ...
        event_end = (
            event_start + len(probe_builder.PROTAGONIST_DEATH_EVENT_BYTES)
        )
        self.assertEqual(
        ...
        pointers = tuple(
            int.from_bytes(
                self.source[event_start + offset : event_start + offset + 4],
                "big",
            )
        ...
        self.assertEqual(
            pointers,
            (
                probe_builder.PROTAGONIST_FIRST_TEXT,
                probe_builder.PROTAGONIST_EGBERT_TEXT,
                probe_builder.PROTAGONIST_FINAL_TEXT,
            ),
        )
        self.assertEqual(
            self.source[event_start + 24 : event_start + 30],
            bytes.fromhex("13 FF 15 FF FF FF"),
        )
    ...
    def test_runtime_clear_preserves_fixed_data_and_targets_only_hostiles(self):
        ...
        self.assertEqual(probe_builder.RUNTIME_CLEAR_GROUPS, tuple(range(10, 20)))
    ...
    def test_runtime_clear_conflicts_with_other_diagnostic_modes(self):
        for arguments in (
            ...
            {"battle_ui_target_only": True, "runtime_clear": True},
