...
class RomVersionTests(unittest.TestCase):
    def test_pure_profile_is_translation_only_release(self):
        ...
        self.assertEqual(profile["release_id"], "ko-original-1.3.5")
        self.assertEqual(profile["translation_version"], "1.3.5")
        self.assertIsNone(profile["balance_version"])
        self.assertEqual(profile["title_text"], "번역:1.3.5")
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "LANGRISSER II KOREAN PURE T1.3.5 BY HSP1324",
        )
    ...
    def test_normal_profile_is_latest_release(self):
        ...
        self.assertEqual(profile["status"], "released")
        self.assertEqual(profile["release_id"], "ko-normal-1.3.5")
        self.assertEqual(profile["translation_version"], "1.3.5")
        self.assertIsNone(profile["balance_version"])
        self.assertEqual(profile["title_text"], "번역:1.3.5")
        self.assertEqual(
        ...
        self.assertEqual(
            profile["header_title"],
            "LANGRISSER II KOREAN T1.3.5 BY HSP1324",
        )
        self.assertEqual(profile["creator"], "hsp1324")
        self.assertEqual(profile["base_release"], "ko-1.3.4")
        self.assertEqual(profile["save_format"], "lang2-ko-sram-v1")
    ...
    def test_hard_profile_is_the_standard_hard_release(self):
        ...
        self.assertEqual(hard["translation_version"], "1.3.5")
        self.assertEqual(hard["balance_version"], "1.3.5")
        self.assertEqual(
            hard["title_text"],
            "번역/밸런스:1.3.5/1.3.5",
        )
        ...
        self.assertEqual(
            hard["header_title"],
            "LANGRISSER II KOREAN T1.3.5 B1.3.5 BY HSP1324",
        )
    ...
    def test_released_hard_profile_uses_dual_version_title(self):
        ...
        self.assertEqual(
            profile["rom_filename"],
            "Langrisser II (Korean Hard T1.3.5 B1.0.0).md",
        )
        self.assertEqual(
            profile["header_title"],
            "LANGRISSER II KOREAN T1.3.5 B1.0.0 BY HSP1324",
        )
