#!/usr/bin/env python3
"""Rebuild and verify the v1.3.9 Japanese-ROM BPS patches."""
...
TARGET_DESCRIPTIONS = {
    "pure": "원작 맵 디자인과 밸런스에 한국어화 및 공통 진행 수정 적용",
    "normal": "최신 New 클래스 디자인과 한국어화 적용",
    "hard": "최신 New 클래스 디자인·한국어화·하드 밸런스 적용",
}
        ...
        records.append(
            {
                ...
                "patch_filename": f"{profile_name if profile_name != 'pure' else 'original'}-{VERSION}.bps",
            }
        )
...
def _verify_version_registry() -> None:
    expected = {
        "pure": ("ko-original-1.3.9", "1.3.9", None, "ko-original-1.3.8"),
        "normal": ("ko-normal-1.3.9", "1.3.9", None, "ko-normal-1.3.8"),
        "hard": ("ko-hard-1.3.9", "1.3.9", "1.3.9", "ko-hard-1.3.8"),
    }
