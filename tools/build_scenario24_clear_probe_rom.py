...
PROTAGONIST_DEATH_TRIGGER = 0x1AF564
PROTAGONIST_DEATH_TRIGGER_BYTES = bytes.fromhex(
    "02 02 01 00 00 1A F7 1A"
)
PROTAGONIST_DEATH_EVENT = 0x1AF71A
PROTAGONIST_DEATH_EVENT_BYTES = bytes.fromhex(
    "02 01 02 01 00 1A FB DA 13 FF 15 FF FF FF"
)
PROTAGONIST_DEATH_TEXT = 0x1AFBDA
PROBE_AT = 0
PROBE_DF = 0
    ...
    for label, offset, expected_bytes in event_spans:
        ...
        for rom_label, data in (("Japanese", source), ("input", probe)):
            if data[offset:end] != expected_bytes:
                raise ValueError(
                    f"{rom_label} Scenario 24 {label} changed"
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "preserve every Scenario 24 deployment and fixed record, then "
...
def main() -> int:
    ...
    if args.protagonist_death:
        print(
            "stock protagonist-death trigger, event, dialogue pointer, and "
