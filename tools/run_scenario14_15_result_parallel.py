#!/usr/bin/env python3
"""Run normal/hard Scenario 14/15 result captures in parallel Xvfb workers."""
...
from tools import run_scenario14_15_result_surface as result_runner


DEFAULT_PROBE_ROOT = ROOT / "tmp/current-result-probes"


def task_rom(probe_root: Path, profile: str, scenario: int) -> Path:
    return probe_root / profile / f"s{scenario:02d}.md"
...
def run_one(
    ...
    output = args.output_root / profile / f"s{scenario:02d}" / args.run_id
    command = [
        sys.executable,
        str(ROOT / "tools/run_scenario14_15_result_surface.py"),
        "--profile", profile,
        ...
        "--settle-delay", str(args.settle_delay),
        "--button-delay", str(args.button_delay),
    ]
    ...
    return {
        ...
        "result_frame": payload.get("result_frame") if payload else None,
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                rows.append(row)
                print(
                    f"{profile} Scenario {scenario}: {row['status']}",
                    flush=True,
                )
    finally:
...
def parse_scenarios(value: str) -> list[int]:
    ...
    if not scenarios or any(
        scenario not in result_runner.SCENARIO_MOVE_DIRECTIONS
        for scenario in scenarios
    ):
        raise argparse.ArgumentTypeError("scenarios must be 14 and/or 15")
...
def main() -> int:
    ...
    parser.add_argument("--scenarios", type=parse_scenarios, default=[14, 15])
    parser.add_argument("--probe-root", type=Path, default=DEFAULT_PROBE_ROOT)
    parser.add_argument("--seed-gst", type=Path, default=matrix.DEFAULT_SEED_GST)
    parser.add_argument("--workers", type=int, default=4)
    parser.add_argument("--display-base", type=int, default=370)
    ...
    parser.add_argument("--output-root", type=Path, default=result_runner.DEFAULT_OUTPUT_ROOT)
