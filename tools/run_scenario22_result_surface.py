#!/usr/bin/env python3
"""Capture current Scenario 22 result and save surfaces from a fresh entry."""
...
DEFAULT_OUTPUT_ROOT = ROOT / "captures/run/current_s22_result"
GST_WORK_RAM_OFFSET = 0x2478
WORK_RAM_BYTES = 0x10000
LIANA_RUNTIME_GROUP = 8
...
def runtime_clear_state(path: Path, before_path: Path) -> dict[str, object]:
    ...
    before_groups = runtime_groups(before_path)
    liana = groups[str(LIANA_RUNTIME_GROUP)]
    liana_before = before_groups[str(LIANA_RUNTIME_GROUP)]
    return {
        "groups": groups,
        "groups_before_start": before_groups,
        "combat_groups_defeated": all(
        ...
        "liana_untouched_by_wrapper": liana == liana_before,
        "liana_runtime_identity_valid": (
            liana["class_id"] == 0x02
            and liana["name_id"] == 0x02
            and not (liana["defeated_flag"] & 0x80)
            and liana["hp"] == 10
            and liana["x"] == 14
            and liana["y"] == 4
        ),
...
def run_capture(args: argparse.Namespace) -> dict[str, object]:
    ...
    runtime_name = f"s22-result-{args.profile}-{args.run_id}"
    ...
    try:
        ...
        before_clear_gst = recorder.save_gst("states/before_runtime_clear.gst")
        ...
        clear_state = runtime_clear_state(clear_gst, before_clear_gst)
        if not clear_state["combat_groups_defeated"]:
            raise RuntimeError("Start wrapper did not defeat every combat group")
        if not clear_state["liana_untouched_by_wrapper"]:
            raise RuntimeError("Start wrapper changed the allied Liana group")
        if not clear_state["liana_runtime_identity_valid"]:
            raise RuntimeError("allied Liana runtime identity is unexpected")
        ...
        report = {
            ...
            "before_runtime_clear_gst": shared.relative(before_clear_gst),
...
def main() -> int:
    ...
    print(
        f"{report['status']}: {args.profile} Scenario 22 result at "
