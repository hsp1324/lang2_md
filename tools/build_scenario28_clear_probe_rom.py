...
DEFAULT_OUTPUT_ROM = ROOT / "roms/builds/Langrisser II (Scenario 28 Clear Probe).md"
SCENARIO_NUMBER = 28
SCENARIO_HEADER = 0x183286
DEPLOYMENT_POINTER_OFFSET = 0x08
DEPLOYMENT_TABLE = 0x1832A6
...
GENERIC_BROTHER_RECORD_INDEX = 0
ADON_RECORD_INDEX = 3
SAMSON_RECORD_INDEX = 7
BARAN_RECORD_INDEX = 8
PROTAGONIST_DEATH_TRIGGER = 0x1B5554
PROTAGONIST_DEATH_TRIGGER_BYTES = bytes.fromhex(
    "0F 02 01 00 00 1B 59 A8"
)
PROTAGONIST_DEATH_EVENT = 0x1B59A8
PROTAGONIST_DEATH_EVENT_BYTES = bytes.fromhex(
    "02 01 02 01 00 1B 65 92 13 FF 15 FF FF FF"
)
PROTAGONIST_DEATH_TEXT = 0x1B6592
...
COMPLETION_TARGET_RUNTIME_GROUP = FIRST_FIXED_RUNTIME_GROUP + BARAN_RECORD_INDEX
...
ACCEPTED_COMPLETION_CHECKSUM = 0x94FE
CURRENT_COMPLETION_CHECKSUM = 0xF7E8
CURRENT_PROTAGONIST_DEATH_CHECKSUM = 0x5A95
...
def completion_hp_wrapper_code() -> bytes:
    code = bytearray()
    # Opening events have already materialized every fixed brother group when
    # Start is opened. Remove only the non-target runtime copies so all source
    # records and their dialogue/event handlers remain intact.
...
def validate_layout(probe: bytes, source: bytes) -> None:
    ...
    if source_layout != probe_layout:
        raise ValueError("Scenario 28 layout differs from Japanese source")
    if source_layout.header_offset != SCENARIO_HEADER:
        raise ValueError(
            f"unexpected Scenario 28 header 0x{source_layout.header_offset:06X}"
        )
    if source_layout.record_count != 9:
        raise ValueError(
            f"unexpected Scenario 28 fixed record count {source_layout.record_count}"
        )
    if be32(source, SCENARIO_HEADER + DEPLOYMENT_POINTER_OFFSET) != DEPLOYMENT_TABLE:
        raise ValueError("unexpected Japanese Scenario 28 deployment table")
    ...
    for label, data in (("Japanese source", source), ("input ROM", probe)):
        if data[FIRST_PLAYER_DEPLOYMENT_OFFSET:end] != expected:
            raise ValueError(f"{label} Scenario 28 player deployments differ")
    for index in range(source_layout.record_count):
        ...
        if probe[base:end] != source[base:end]:
            raise ValueError(
                f"input Scenario 28 fixed record {index} differs from Japanese source"
            )
    for label, offset, expected_bytes in (
        ...
        for rom_label, data in (("Japanese", source), ("input", probe)):
            if data[offset:end] != expected_bytes:
                raise ValueError(
                    f"{rom_label} Scenario 28 {label} changed"
                )
...
def patch_probe(
    ...
    *,
    ...
    if completion_target_only and protagonist_death:
        raise ValueError("Scenario 28 diagnostic modes are mutually exclusive")
    ...
    if completion_target_only:
        target_base = (
            layout.records_offset + BARAN_RECORD_INDEX * FIXED_RECORD_SIZE
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build an ignored Scenario 28 ROM with weakened Builder groups "
            "while preserving all brother identities, stock deployments, "
            "and event handlers"
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "stage only Baran above the stock first Elwin deployment, then "
            "remove runtime groups 7..14 and lower group 15 Baran to one HP "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "preserve every fixed record and mark only runtime player group 0 "
            "defeated when Start is opened"
        ),
    )
...
def main() -> int:
    ...
    print("Scenario 28 enemy records 0..8: AT 0, DF 0, no mercenaries")
    print(
        "stock deployments, brother identities, Builder classes, levels, "
        "and all handlers preserved"
    )
    if args.completion_target_only:
        print(
            "stock seven-commander preparation layout retained; only fixed "
            f"Baran is staged at {COMPLETION_TARGET_POSITION}"
        )
        print(
            "Start hides and defeats runtime groups 7..14, then lowers only "
            "present, living group 15 Baran to one HP"
        )
    elif args.protagonist_death:
        print(
            "all source deployments and fixed records preserved; Start marks "
            "only runtime player group 0 defeated"
        )
