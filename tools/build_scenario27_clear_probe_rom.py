...
DEFAULT_OUTPUT_ROM = ROOT / "roms/builds/Langrisser II (Scenario 27 Clear Probe).md"
SCENARIO_NUMBER = 27
...
DEMON_LORD_RECORD_INDEX = 0
BERNHARDT_RECORD_INDEX = 8
HIDDEN_LEON_RECORD_INDEX = 9
PROBE_AT = 0
PROBE_DF = 0
...
def validate_layout(probe: bytes, source: bytes) -> None:
    ...
    for label, data in (("Japanese source", source), ("input ROM", probe)):
        if data[FIRST_PLAYER_DEPLOYMENT_OFFSET:end] != expected:
            raise ValueError(f"{label} Scenario 27 player deployments differ")
    for index in range(source_layout.record_count):
        ...
        if probe[base:end] != source[base:end]:
            raise ValueError(
                f"input Scenario 27 fixed record {index} differs from Japanese source"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build an ignored Scenario 27 ROM with weakened enemy groups "
            "while preserving Bernhardt, hidden Leon, stock deployments, "
...
def main() -> int:
    ...
    print("Scenario 27 enemy records 0..9: AT 0, DF 0, no mercenaries")
    print(
        "stock deployments, sides, identities, classes, levels, hidden Leon, "
