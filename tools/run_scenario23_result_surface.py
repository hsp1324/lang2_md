#!/usr/bin/env python3
"""Capture current Scenario 23 result and save surfaces from a fresh entry."""
...
DEFAULT_OUTPUT_ROOT = ROOT / "captures/run/current_s23_result"
GST_WORK_RAM_OFFSET = 0x2478
WORK_RAM_BYTES = 0x10000
FIRST_PLAYER_RUNTIME_GROUP = 0
...
def runtime_clear_state(path: Path, before_path: Path) -> dict[str, object]:
    ...
    return {
        "groups": groups,
        "groups_before_start": before,
        "player_groups_untouched_by_wrapper": all(
            groups[str(group)] == before[str(group)] for group in players
        ),
        "hostile_groups_defeated": all(
            groups[str(group)]["defeated_flag"]
            == (before[str(group)]["defeated_flag"] | 0x80)
            and groups[str(group)]["hp"] == 0
            and groups[str(group)]["x"] == 0xFF
            and groups[str(group)]["class_id"]
            == before[str(group)]["class_id"]
            and groups[str(group)]["name_id"] == before[str(group)]["name_id"]
            and groups[str(group)]["y"] == before[str(group)]["y"]
            for group in hostiles
        ),
    }
...
def run_capture(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        ...
        if not clear_state["player_groups_untouched_by_wrapper"]:
            raise RuntimeError("Start wrapper changed a player runtime group")
        if not clear_state["hostile_groups_defeated"]:
