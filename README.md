...
- 아이템 보유 화면 제목 `アイテム所持`는 ROM에 `0000 0001 ...` 형태의 로컬 토큰 시퀀스로 여러 번 들어 있습니다. 이 토큰을 `0x701E...` 같은 전역 glyph 번호로 직접 바꾸면 상단 제목이 아이콘/타일 쓰레기로 깨집니다. 기본 빌드는 토큰 시퀀스는 그대로 두고, 재배치한 아이템 glyph 목록 `0x282000`의 앞 6개 슬롯을 `아이템소지 + 공백`으로 교체해 제목을 한국어로 표시합니다.
- 2026-07-09 빌드 `B578`에서 상점 구매 후 보유 화면의 상단 제목은 `아이템소지`로 확인했습니다. 중앙 보유 팝업은 아직 `지`만 표시되는 별도 타일/좌표 문제가 남아 있습니다.
...
- 지휘관 배치 메뉴는 일부 행이 일반 직접 문자열과 다르게 VRAM 타일 그래픽/다른 참조를 섞어 사용합니다. 2026-07-09 빌드 `AF5A`에서 `지휘관배치`, `적군보기`, `출격`은 확인했고, 2~3행 `移動順変更`, `自動配置` 일본어 잔존 문제가 남아 있어 별도 VRAM 기준 분석이 필요합니다.
- ROM 초반의 `SCENARIO` 시스템 메뉴 문자열은 `LOAD/SAVE/CONTINUE/SCENARIO/NOTHING` 테이블과 붙어 있어 준비창 `SCENARIO 1`과 별개이므로 건드리지 않습니다.
