...
- `tools/jp_short_inline_byte_inventory.py`: 위 스캔이 의도적으로 제외한 의미 바이트 1~2개의 짧은 `FF` 종료 후보 6,612개를 영역별로 집계합니다. 클래스/스프라이트/아이템/이름/그래픽/시스템/엔딩/시나리오 및 텍스트/UI 뱅크의 401개는 16비트 문맥과 직접·PC 상대 참조를 대조해 400개 포인터 경계·글리프 워드·그래픽·레이아웃 오탐과 시나리오 설명 병력 레벨 접두사 `L-`로 분류합니다. `0x050000..0x07FFFF`에서 포인터처럼 보인 다섯 개의 4바이트 창도 수치/그래픽 인덱스 행 내부 조합 두 건과 68000 명령/피연산자 경계 조합 세 건으로 판독해 실제 포인터에서 제외합니다. 나머지는 소유권을 추정하지 않으며 base-relative·indexed·동적 접근 가능성을 열린 조사 공백으로 유지합니다.
