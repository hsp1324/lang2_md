...
- 지휘관 이름과 클래스/용병명은 CP932 반각 가나 바이트 문자열 경로입니다. 1장에 필요한 `엘윈`, `리아나`, `헤인`, `볼도`, `파이터`, `워록`, `솔저`, `가드맨`, `단검`과 아이템 탭 `무기/방어구/아이템` 등의 문자열 바이트 패치 코드는 남겨 두었지만, 기본 빌드에서는 꺼 둡니다. 실제 준비창/상점의 작은 8x8 글리프는 `0x40000`의 16x16 일본어 폰트가 아니라 별도 시스템 폰트가 VRAM으로 로드되는 경로라, 문자열만 한글 바이트로 바꾸면 화면에서는 관련 없는 일본어 글자로 보입니다. 이 폰트 원본 위치를 먼저 추적 중입니다.
...
- `WEPON`, `PROTECTER`, `ITEM`, `POINT`, `END` 같은 일부 ASCII 라벨은 별도 1바이트 고정 문자열입니다. 현재는 작은 8x8 시스템 폰트 원본을 찾기 전까지 기본 빌드에서 이 경로를 패치하지 않습니다. ROM 초반의 `SCENARIO` 시스템 메뉴 문자열은 `LOAD/SAVE/CONTINUE/SCENARIO/NOTHING` 테이블과 붙어 있어 준비창 `SCENARIO 1`과 별개이므로 건드리지 않습니다.
...
- 작은 UI 폰트 추적에는 GST 저장 상태의 VRAM을 직접 렌더링하는 `tools/render_md_vram_tiles.py`를 사용합니다. GST 파일의 VRAM 영역은 현재 `0x12478`부터 64KB로 확인했습니다.
