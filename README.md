...
- 첫 타이틀의 저작권 문구 아래에는 `한글화: HSP1324`를 표시합니다. 원본 타이틀은 정상 소문자 글꼴을 갖지 않고 해당 코드 일부를 지형/아이콘으로 재사용하므로 아이디는 원본 대문자/숫자 글꼴로 통일합니다. 타이틀에서만 압축 리소스 436을 VRAM 타일 `0x3A..0x73`에 덮어쓰고 다음 화면의 기존 글꼴 로더가 게임용 슬롯을 복원합니다. 같은 전용 리소스의 미사용 `a..g` 코드 위치를 이용해 첫 메뉴를 `새 게임 / 불러오기`로 바꾸며 원본 `PUSH START BUTTON`, 저작권, 게임 내 이름·클래스 글꼴은 보존합니다. 대형 로고는 사용자가 제공한 영문판 ROM의 실제 리소스 393을 사용합니다. 영문판 타입 0 원시 payload(`0x1E6E00`, 5984바이트)와 28×8 배치(`0x0A428E`, 사용 231바이트)를 해시로 검증하고, 일본판 호출 구조에 맞춰 타입 3으로 재압축해 `0x2E0000`에 배치하므로 장식체 `Langrisser`, 대형 `II`, 천사 배경이 영문판과 같습니다. production checksum `45D8`의 `captures/run/45d8_title_english_logo_prompt_current.png`에서 로고·`PUSH START BUTTON`·대문자 크레딧을 실기 확인했습니다.
