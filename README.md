...
- `tools/build_scenario14_clear_probe_rom.py`: 시나리오 14의 일본판 헤더·7명 배치표·11개 적 레코드와 엘윈·쉐리·제시카의 원본 랑그릿사 도달 트리거를 검증하고 적 AT/DF·용병만 제한합니다. 기본 모드는 모든 배치를 보존하며, `--completion-layout`은 엘윈만 원본 목표 `(16,6)` 바로 아래 `(16,7)`로 옮기는 비배포 진단입니다. checksum `B658`에서 정상 이동으로 랑그릿사를 얻고 레온 퇴각·아군 후속·전과보고·15장 저장·진입까지 확인했습니다. 현재 대사 수정 뒤 동일 파생 체크섬은 `9CFF`/`9CE5`이며, 배치와 제한 수치는 편집기 기본값의 근거로 사용하지 않습니다.
