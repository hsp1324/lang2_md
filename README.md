...
- 지휘관 이름과 클래스/용병명은 CP932 반각 가나 바이트 문자열 경로입니다. 클래스 포인터 테이블 `0x05E6D8`의 일본어 원문을 빌드 중 검증한 뒤 1장에 필요한 `파이터`, `워록`, `솔저`, `가드맨`, `헤비호스맨`, `로얄호스` 등을 제한 패치합니다. 외형만 보고 `기병/중기병`처럼 일반화하지 않습니다. 안전한 한글 슬롯 60개를 넘기지 않기 위해 작은 탭은 `WPN/ARMOR/ITEM`, 금액은 `소지G`를 사용합니다.
...
- `WEPON`, `PROTECTER`, `ITEM`, `POINT`, `END` 같은 일부 ASCII 라벨은 별도 1바이트 고정 문자열입니다. 현재 작은 탭은 안전한 한글 슬롯을 정확한 클래스명에 우선 배정하기 위해 `WPN/ARMOR/ITEM`을 사용하고, 금액은 `소지G`로 표시합니다. 이 경로를 16x16 직접 glyph 시퀀스로 바꾸면 준비창 왼쪽 아래 금액 라벨이 빨간 깨짐/빈 타일로 표시됩니다.
