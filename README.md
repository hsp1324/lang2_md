...
- 2026-07-14 실기 확인에서 27장 준비창의 `셰리/아론/레스터`가 깨져 보이는 회귀를 확인했습니다. 이름 포인터 `0x0618E8`의 미패치 일본어 바이트가 부분 교체된 8x8 글꼴로 출력되는 문제이며 세이브 손상이 아닙니다. 현재 안전 글리프 64칸이 모두 사용 중이므로 다른 검증 UI 글자를 밀어내는 임시 추가는 금지하고, 화면별 글꼴 뱅크 또는 렌더러 확장으로 해결합니다. 상세 주소와 실패 조건은 `HANDOFF.md`의 `Later-Scenario Commander Name Regression`에 기록했습니다.
