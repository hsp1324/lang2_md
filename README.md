...
브라우저에서 `http://127.0.0.1:8765`를 엽니다. 탭에서 31개 시나리오의 클래스·LV·AT·DF·용병 6칸, 37개 아이템의 가격·효과 4칸, 10명 지휘관의 클래스 체인지 경로와 전직 시 새로 해금할 용병을 수정할 수 있습니다. 시나리오와 클래스 트리에는 ROM에서 직접 추출한 클래스/용병 그림이 표시되고, 아이템 표에는 실기 캡처로 검증한 원본 아이콘이 표시됩니다. 용병 선택기는 157개 클래스 ID의 그림과 한국어·일본어 이름을 함께 보여줍니다. `편집 ROM 빌드`는 원본을 덮어쓰지 않고 다음 파일을 만듭니다.
