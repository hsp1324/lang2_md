...
- `tools/build_scenario5_escape_probe_rom.py`: 기본 모드는 시나리오 5의 `20턴 내 북쪽 도착`을 원본 이벤트로 재현하기 위해 첫 엘윈 배치의 Y만 `50→1`로 바꿉니다. `--enemy-annihilation`은 원본 적 9명의 정체·클래스·레벨·좌표·이벤트를 보존하고 AT/DF·용병만 제한한 뒤, Start 진입 때 런타임 적 8명을 제거하고 남은 원본 늑대인간의 HP만 1로 낮춥니다. 체크섬 `398C`의 북쪽 도착과 `B5E8`의 적 전멸 모두 정상 명령으로 전용 대사·전과보고·6장 저장까지 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
