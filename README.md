...
> 최신 배포 버전은 `v1.4.2`입니다. 세 가지 완성판은
...
- **Windows**: [Langrisser-II-Korean-Patcher-v1.4.2.exe](https://github.com/hsp1324/lang2_md/releases/download/v1.4.2/Langrisser-II-Korean-Patcher-v1.4.2.exe)
- **Linux x86_64**: [Langrisser-II-Korean-Patcher-v1.4.2-linux-x86_64.tar.gz](https://github.com/hsp1324/lang2_md/releases/download/v1.4.2/Langrisser-II-Korean-Patcher-v1.4.2-linux-x86_64.tar.gz)
- **Linux ARM64**: [Langrisser-II-Korean-Patcher-v1.4.2-linux-arm64.tar.gz](https://github.com/hsp1324/lang2_md/releases/download/v1.4.2/Langrisser-II-Korean-Patcher-v1.4.2-linux-arm64.tar.gz)
- **macOS Apple Silicon**: [Langrisser-II-Korean-Patcher-v1.4.2-macos-arm64.app.zip](https://github.com/hsp1324/lang2_md/releases/download/v1.4.2/Langrisser-II-Korean-Patcher-v1.4.2-macos-arm64.app.zip)
- **macOS Intel**: [Langrisser-II-Korean-Patcher-v1.4.2-macos-x86_64.app.zip](https://github.com/hsp1324/lang2_md/releases/download/v1.4.2/Langrisser-II-Korean-Patcher-v1.4.2-macos-x86_64.app.zip)

검증값은 [v1.4.2 Release](https://github.com/hsp1324/lang2_md/releases/tag/v1.4.2)에서 확인할 수 있습니다.

### v1.4.2 변경 사항

- Normal·Hard에서 시나리오 1~25와 비밀 시나리오 X1~X3을 정상적으로
  완료하고 아군 퇴각 조건을 지킨 경우, 시나리오 26을 처음 클리어할 때도
  에그베르트의 비밀 시나리오 X4 제안이 나오도록 수정했습니다. 시나리오 26을
  이미 한 번 클리어했어야 하는 불가능한 선행 조건만 제거했습니다.
- Original은 원작의 X4 진입 조건을 그대로 유지합니다.
...
- 시나리오 1 프롤로그에서 `어느 날`이 `어` / `느 날`로 갈라지던 고정
  줄바꿈 위치를 바로잡았습니다.
- Original은 키스·레스터·제시카가 원작과 똑같이 2단계 클래스와 원래
  레벨로 등장하도록 복원했습니다. 최신 디자인과 합류 선택 로직도 적용하지
  않은 원작 게임 구성의 한국어판입니다.
- Normal은 최신 New 클래스 디자인을 사용하며 키스·레스터·제시카가 합류할 때
  2단계 클래스를 직접 고를 수 있도록 1단계 LV10으로 등장합니다. 클래스 선택
  뒤 지급되는 경험치량은 원작에서 각 인물이 가진 경험치량과 같습니다.
- Hard의 모든 일반·비밀 상점에 룬스톤과 경험치 2배 효과의 메사이어소드를
  항상 판매하도록 추가했습니다.
- 마지막 엔딩 크레딧의 번역자 이름을 `홍선표(hsp1324)`로 표기합니다.

### v1.3.9 변경 사항

- 전장 지휘관 명령의 마지막 항목을 `자동`이 아닌 `수동`으로 수정했습니다.
- 상점의 `크라운`, `천사날개`, `그레이프니르` 등 긴 아이템 이름이 깨지던 문제를
  상점 설명 DMA 뒤 안전한 이름 글꼴 영역을 다시 읽는 방식으로 수정했습니다.
  상점이 사용하는 B400/B700/B800 그래픽 영역은 건드리지 않습니다.
- 시나리오 1 서장 문장을 `헤인의 소꿉친구 리아나`, `엘윈은 리아나를 구하기 위해
  검을 들었다`로 바로잡았습니다.
- 시나리오 15에서 적 지휘관 격파 뒤 대사가 섞이거나 한 줄씩 밀리던 일본판 이벤트
  페이지 대응을 수정했습니다.
- 레스터 아크메이지의 숨은 `자베러` 전직 경로와 마법 무효 시 `프린`으로 보이던
  원래 `ミス` 글리프 충돌을 수정했습니다.
...
- Original·Normal·Hard를 각각 1장부터 비밀 시나리오를 포함한 마지막 27장과
  ...
  재검증은 계속합니다. v1.3.8은 이 검증본의 게임 로직을 유지하고 로렌의
  최신 디자인 맵 팔레트만 빨간색 명암으로 교정했습니다.

## 세 가지 버전

- **원작 한국어판(Original)**: 원작의 디자인·밸런스·합류 클래스를 그대로
  유지하고 한국어화만 적용합니다. 키스·레스터·제시카도 원작처럼 2단계
  클래스와 원래 레벨로 등장합니다.
- **최신 디자인 일반판(Normal)**: New 디자인의 최신 사용자 디자인과
  한국어화를 적용하며 난이도는 원작 기준입니다. 키스·레스터·제시카는 합류할
  때 2단계 클래스를 직접 고를 수 있도록 1단계 LV10으로 등장하고, 선택 뒤
  받는 경험치량은 원작과 같습니다.
- **최신 디자인 하드판(Hard)**: 최신 사용자 디자인과 한국어화에 적 지휘관·
  용병 강화 밸런스를 더하며 모든 상점에서 룬스톤과 메사이어소드를 판매합니다.

현재 v1.4.2 패처는 일본판 ROM 또는 ZIP 하나를 고르면 위 세 ROM을 모두
생성합니다.
...
```bash
tar -xzf Langrisser-II-Korean-Patcher-v1.4.2-linux-x86_64.tar.gz
chmod +x Langrisser-II-Korean-Patcher-v1.4.2-linux-x86_64
./Langrisser-II-Korean-Patcher-v1.4.2-linux-x86_64
```
...
ARM64에서는 명령의 파일명을 `Langrisser-II-Korean-Patcher-v1.4.2-linux-arm64`로
바꿉니다.

## macOS에서 패치하기

M1 이후 Mac은 `macos-arm64.app.zip`, Intel Mac은 `macos-x86_64.app.zip`을
받습니다. ZIP을 풀고 `Langrisser II Korean Patcher v1.4.2.app`을 실행합니다.
...
```bash
"./Langrisser II Korean Patcher v1.4.2.app/Contents/MacOS/Langrisser II Korean Patcher v1.4.2" \
...
확인해 주세요. Normal/Hard v1.4.2 ROM은 공개 구 버전 세이브의 키스·레스터가
...
예를 들어 새 ROM이 `Langrisser II (Korean Normal v1.4.2).md`이면 다음 저장
파일이 생성됩니다.

```text
Langrisser II (Korean Normal v1.4.2).srm
```
...
- [v1.4.2 릴리스 노트](docs/release_notes_v1.4.2.md)
- [v1.4.2 검증 기록](docs/v1.4.2_validation.md)
- [v1.4.2 패치 배포 안내](docs/player_patch_distribution_v1.4.2.md)
...
- [v1.3.9 릴리스 노트](docs/release_notes_v1.3.9.md)
