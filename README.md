...
Release 패키지의 `migrate_save.bat`를 사용하면 기존 파일을 직접 이름
변경할 필요가 없습니다.

1. 기존 버전에서 게임 내 저장을 합니다.
2. 에뮬레이터를 완전히 종료합니다.
3. `migrate_save.bat`를 실행합니다.
4. 기존 `.srm` 파일과 새 ROM을 차례로 지정합니다.
5. 도구가 기존 `.srm`을 그대로 보존하고, 같은 저장 폴더에 새 ROM과
...
대상 이름에 다른 저장 파일이 이미 있으면 자동으로 덮어쓰지 않고
중단합니다. 기존 한국어판 ROM 자체를 같은 파일명으로 업데이트하는
`apply_update.bat` 방식은 `.srm` 이름을 바꿀 필요가 없습니다.
...
- **세이브가 보이지 않음**: `migrate_save.bat`를 실행하고 ROM과 `.srm`의 기본 파일명이 같은지 확인합니다.
