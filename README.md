...
- `tools/build_scenario20_clear_probe_rom.py`: 시나리오 20의 일본판 헤더·8명 배치표·적군 10명을 검증하고 AT/DF·용병만 제한합니다. 파이어스와 숨은 와이번·크라켄의 정체·클래스·레벨·좌표 및 키스 관련 해상 이벤트를 보존합니다. `--completion-layout`은 이벤트가 참조하는 파이어스의 원본 레코드 인덱스 5를 유지한 채 다른 9개 레코드를 숨기고 엘윈만 바로 위 `(22,22)`로 옮기는 비배포 진단입니다. checksum `D2F9`에서 정상 전투 두 번으로 파이어스를 격파하고 전과보고, 21장 저장·진입까지 확인했습니다.
