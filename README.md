...
- `tools/build_scenario9_clear_probe_rom.py`: 기본 모드는 시나리오 9의 레아드 한 명만 원본 엘윈 배치 바로 위로 옮기고 AT/DF와 용병을 제한합니다. `--npc-annihilation`은 원본 고정 레코드를 모두 보존하고 NPC 런타임 그룹 `7..9`만 패배 처리하며, `--protagonist-death`는 엘윈 그룹만 패배 처리합니다. 레아드 격파·NPC 전멸·주인공 사망의 전용 대사와 GAME OVER, 정상 경로의 전과보고·10장 저장과 진입까지 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
