...
- 시나리오 클리어 결과 제목 `戦果報告`는 전역 글꼴의 `戦/果/報/告` ID를 나열한 `0x0A2D88`의 유일한 4워드 목록입니다. 원본 네 ID와 뒤의 `FFFF`를 검증한 뒤 `전과보고`로 교체합니다. production checksum `AD01`에서 글리프 비트맵·종료값 정적 검증은 완료했으며, 새 체크섬의 결과 화면 실기 재캡처는 남아 있습니다.
