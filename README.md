...
이 실험 옵션들은 기본 빌드에서 꺼져 있습니다. `--patch-class-byte-table`과 `--patch-class-byte-subset`은 클래스/용병명 1바이트 문자열을 건드리지만, 현재 준비창의 클래스 표시가 별도 1바이트 표시 경로를 타기 때문에 기본 ROM에서는 쓰지 않습니다. `--include-unsafe-direct-names`는 `0x974xx` 후보 이름 테이블을 건드려 이름 확정 이후 진행을 깨뜨릴 수 있습니다.
...
- 지휘관 이름과 클래스/용병명은 아직 완전 해결되지 않았습니다. 이 영역은 CP932 반각 가나 바이트 문자열처럼 보이지만 실제 표시 폰트 경로가 시나리오용 `0x40000` glyph 경로와 다릅니다.
