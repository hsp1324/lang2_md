...
- `tools/build_scenario24_clear_probe_rom.py`: 시나리오 24의 일본판 헤더·9명 배치표·11개 고정 레코드를 검증합니다. 기본 모드는 특수 진영 베른하르트를 통째로 보존하고 데몬로드·리치·케르베로스·뱀파이어로드 적군 10명의 AT/DF·용병만 제한합니다. `--completion-target-only`는 이벤트가 되살리는 런타임 그룹을 Start에서 숨김·격파 처리하고 원본 뱀파이어로드만 엘윈 옆 공격 대상으로 남겨 완료 대사·전과보고·25장 저장을 검사하는 비배포 진단입니다.
