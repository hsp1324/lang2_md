...
- 아이템 보유 화면 제목 `アイテム所持`는 ROM에 `0000 0001 ...` 형태의 로컬 토큰 시퀀스로 여러 번 들어 있지만, 이 토큰은 전역 glyph 번호가 아니라 화면별 glyph 목록 인덱스입니다. `0x701E...` 같은 전역 한글 glyph 번호를 직접 넣거나 아이템명용 `0xA14AC` glyph 목록 인덱스로 바꾸면 상단 제목이 아이콘/타일 쓰레기로 깨집니다. 기본 빌드에서는 이 제목을 원본으로 유지하고, 제목 전용 glyph 목록 소유권을 찾은 뒤 별도 패치해야 합니다.
