...
- 2026-07-14에는 27장 준비창의 `셰리/아론/키스` 회귀를 15타일 확장 8x8 글꼴 뱅크로 해결했습니다. 이름 문자열의 `F0-FE`는 확장 코드로 쓰고 렌더러에서 VRAM `3F0-3FE`로 변환합니다. 왼쪽 지휘관 목록의 실제 빌더는 `0x22496`, 변환 훅은 `0x22502`입니다. checksum `524D`에서 `엘윈/헤인/셰리/아론/키스`와 1장 `엘윈/헤인`, LV 게이지를 실기로 확인했습니다. 기본 글꼴의 `F0-FE` 그래픽을 전역 교체하면 상태 UI가 다시 깨지므로 금지합니다. 상세 주소와 실패 시도는 `HANDOFF.md`의 `Later-Scenario Commander Name Regression`에 기록했습니다.
