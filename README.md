...
- `0x018082`의 `ｱｲﾃﾑ ｾﾝﾀｸ` 문자열은 상점 관련 텍스트처럼 보이지만, 이를 `BYTE_UI_STRING_PATCHES` 앞쪽에 넣으면 1바이트 한글 폰트 코드 배정 순서가 밀려 `엘윈`이 `아이`로 보이는 회귀가 생깁니다. 2026-07-10에 시도 후 되돌렸습니다. 이 경로를 다시 시도한다면 byte-code 배정 순서를 고정하거나 전용 코드로 분리해야 합니다.
