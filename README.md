...
- `tools/build_scenario25_clear_probe_rom.py`: 시나리오 25의 일본판 헤더·9명 배치표·12개 고정 레코드를 검증합니다. 기본 모드는 아군 제시카를 통째로 보존하고 레온·레아드·에그베르트 및 숨은 드래곤로드를 포함한 적군 11명의 AT/DF·용병만 제한합니다. `--completion-target-only`는 개막 이벤트 뒤 레온만 원본 좌표에 남기고 엘윈을 바로 아래에 배치해 정상 전투·전과보고·26장 저장과 진입을 확인하는 비배포 진단입니다.
