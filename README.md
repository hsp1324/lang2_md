...
- 지휘관 이름과 클래스/용병명은 CP932 반각 가나 바이트 문자열 경로입니다. 실제 클래스 포인터 테이블 `0x05E6D6`의 일본어 원문을 빌드 중 검증한 뒤 1장에 필요한 `파이터`, `워록`, `솔저`, `가드맨`, `헤비호스맨`, `로얄호스` 등을 제한 패치합니다. 외형만 보고 `기병/중기병`처럼 일반화하지 않습니다. 장비 탭은 `무기/방어구/장신구`, 금액은 `소지금`을 사용합니다.
- 2026-07-14에는 27장 준비창의 `셰리/아론/레스터` 회귀를 15타일 확장 8x8 글꼴 뱅크로 해결했습니다. 이름 문자열의 `F0-FE`는 확장 코드로 쓰고 렌더러에서 VRAM `3F0-3FE`로 변환합니다. 왼쪽 지휘관 목록의 실제 빌더는 `0x22496`, 변환 훅은 `0x22502`입니다. checksum `524D`에서 `엘윈/헤인/셰리/아론/키스`와 1장 `엘윈/헤인`, LV 게이지를 실기로 확인했습니다. 기본 글꼴의 `F0-FE` 그래픽을 전역 교체하면 상태 UI가 다시 깨지므로 금지합니다. 상세 주소와 실패 시도는 `HANDOFF.md`의 `Later-Scenario Commander Name Regression`에 기록했습니다.
