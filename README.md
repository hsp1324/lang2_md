...
- 아이템/상점 제목은 상점 메시지 레코드와 겹치는 것처럼 보이지만 소유자가 다릅니다. `0xA1716`은 제목이 아니라 `0000 0001 0012 0020` 제어 워드를 가진 `소지불가` 메시지 레코드이므로, 제목으로 덮으면 구매 팝업에 `입` 같은 잔여 글자가 남습니다. 보유/판매 화면 상단 제목은 별도 token-to-VRAM-tile 경로이며, `0xA17B8`은 token을 `0x680 + token*4` 타일 블록으로 직접 배치합니다. 현재 빌드는 `0xA181C`의 판매 제목 슬롯을 `아이템판매`용 한글 glyph로 바꾸되, token stream 자체는 유지합니다. 구매 제목 쪽의 `0xA177E`는 `FFF9` 제어 서브스트림이 섞여 있어 단순 glyph ID 목록처럼 덮으면 상점 진입이 회색/검은 화면으로 깨질 수 있으므로 아직 별도 추적 대상입니다.
