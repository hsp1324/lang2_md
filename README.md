...
- 지휘관 이름과 클래스/용병명은 CP932 반각 가나 바이트 문자열 경로입니다. 기본 빌드는 1장에 필요한 `엘윈`, `리아나`, `헤인`, `볼도`, `파이터`, `워록`, `솔저`, `가드맨`, `단검`과 아이템 탭 `무기/방어구/아이템` 등의 문자열 바이트를 교체합니다. 다만 실제 준비창/상점의 작은 8x8 글리프는 `0x40000`의 16x16 일본어 폰트가 아니라 별도 압축 폰트가 VRAM으로 로드되는 경로라, 현재는 원본 폰트 위치를 추적 중입니다.
