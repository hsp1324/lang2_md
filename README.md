...
- `tools/build_scenario19_clear_probe_rom.py`: 시나리오 19의 일본판 헤더·8명 배치표·적군 10명을 검증하고 AT/DF·용병만 제한합니다. 이멜다와 숨은 레아드·제국 증원의 정체·클래스·레벨·좌표 및 모든 이벤트를 보존합니다. `--completion-layout`은 이멜다를 원본 `(37,23)`에 둔 채 엘윈만 바로 위 `(37,22)`로 옮깁니다. 과거 checksum `2829`와 격리 저장 슬롯 AT/DF 99에서 이멜다 격파·레아드 증원·출항·20장 저장·진입까지 확인했습니다. 현재 생산 ROM `3497` 기준 기본/완료 배치는 `7E18`/`7E48`입니다. `--protagonist-death`는 원본 배치·고정 레코드·`0x1A5E60` 트리거와 `0x1A6144` 핸들러를 보존하고 런타임 엘윈만 사망 처리하며, 현재 체크섬은 `2B66`입니다. `--turn-event {2,13,18,19,21,23}`은 원본 배치·고정 레코드·예약 테이블·핸들러를 모두 보존하고 런타임 아군 8명만 보호한 채 지정 턴으로 진행합니다. 현재 체크섬 `625E`/`6276`/`6280`/`6282`/`6286`/`628A`에서 모든 예약 턴 대사, 다음 턴 복귀, 턴 23의 원본 시간초과 GAME OVER를 실기로 확인했습니다.
