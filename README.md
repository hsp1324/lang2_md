...
- `tools/build_scenario26_clear_probe_rom.py`: 시나리오 26의 일본판 헤더·10명 배치표·적군 10명을 검증하고 AT/DF·용병만 제한합니다. 아크메이지·위저드·세인트·나이트마스터와 에그베르트의 정체·클래스·레벨·좌표 및 이벤트를 보존합니다. `--battle-ui-target-only`는 원본 아크메이지만 일반 전투 대상으로 남겨 전투 수치·지형·이름·클래스 렌더러를 검사하고, `--completion-target-only`는 개막 뒤 원본 에그베르트만 HP 1로 남겨 보상·아이템 폐기·후속 대사·전과보고·27장 저장까지 검사합니다. 두 모드는 원본 고정 레코드를 수정하지 않고 Start 진입 시 런타임 그룹만 격리하는 상호 배타적 비배포 진단입니다.
