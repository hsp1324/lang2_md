...
Production `1AB2`는 전투 클래스 호출 `0x01B546`의 정확한 복귀 주소
`0x01B54C`와 출력 버퍼 `0xFFFFA714`를 함께 확인한다. 호출자가 보존한 좌우
구분값은 동적 렌더러 진입 뒤 스택 `+64`에 있으며, 왼쪽은 기존 안전 캐시
슬롯 `0..7`, 오른쪽은 `8..15`를 사용한다. 따라서 두 클래스명이 동시에
보여도 서로 덮지 않고, 전투 그래픽과 충돌하는 정적 확장 글꼴로도 돌아가지
않는다. `1ab2_s21_current_leviathan_hawknight_battle.png`에서 실제 적 턴의
`리바이어선/호크나이트`, 전투 유닛, `AT/DF/지형`을 함께 확인했다.
