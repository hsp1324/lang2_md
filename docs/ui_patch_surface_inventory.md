...
- exact ownership of low-signal byte sequences outside the 38 reviewed text/UI-bank candidates: the one/two-signal scan region-classifies 6,612 candidates, and proves the 38 text/UI-bank rows are word/layout data with no exact aligned-32-bit or LEA/PEA PC-relative reference; base-relative, indexed, and dynamic access elsewhere remains possible
