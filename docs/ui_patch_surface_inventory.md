...
- exact ownership of low-signal byte sequences outside the 5,153 reviewed font/class/sprite/item/name/graphics/system/ending/scenario/text/UI/compressed-resource/executable-tail candidates: the one/two-signal scan region-classifies 6,612 candidates, proves 5,152 rows are instruction/bitmap/pointer-boundary/word/graphics/layout/compressed payload data, and identifies retained scenario-level prefix L-; exact ownership of the remaining 1,459 executable/numeric candidates and base-relative, indexed, or dynamic access remains open
