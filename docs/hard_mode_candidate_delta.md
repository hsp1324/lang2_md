...
시나리오 31도 시나리오 30의 체크섬-`8674` 저장 슬롯에서 연속
진입했으며, 런타임 예외 없이 하드 대상 9개가 모두 일치했다.

| 시나리오 | 아군 그룹 | 하드 대상 | 실제 적 그룹 | GST SHA-256 |
| ---: | ---: | ---: | --- | --- |
| 31 | 10 | 9/9 | 10~18 | `3121953eb85889321bee4e236c0e9457c31c43f2ddb3f52759d56a4bc23179e9` |

하단의 `SCENARIO ?4`도 원작 비기 스테이지의 맵 상태 표기다. 화면은
`captures/run/hard_8674_s31_entry.png`에 보존한다.

누락돼 있던 시나리오 2와 3도 같은 최신 후보의 유효 저장 슬롯
계보에서 다시 진입했다. 시나리오 2는 시나리오 31 저장 슬롯에서
선택했고, 시나리오 3은 그 시나리오 2 저장 슬롯에서 연속 진입했다.

| 시나리오 | 아군 그룹 | 하드 대상 | 실제 적 그룹 | GST SHA-256 |
| ---: | ---: | ---: | --- | --- |
| 2 | 3 | 6/6 | 7~12 | `1e68a91b3f6c8de0963eafcf53fc43054ace358df3a7182f6c0a09546550f0b6` |
| 3 | 3 | 8/8 | 5~12 | `376bdfecd69a43449fc8e506c027a12c82a14b970681b73f5af7311f985373e5` |

화면은 `captures/run/hard_8674_s02_full_entry.png`와
`captures/run/hard_8674_s03_full_entry.png`에 보존한다. 이로써
시나리오 1의 심층 증거와 시나리오 2~31의 최신 후보 로더 스모크를
합쳐 31개 전 시나리오의 진입 및 하드 대상 데이터 로딩을 확인했다.
이는 각 시나리오를 처음부터 끝까지 플레이한 밸런스 완주 검증을
의미하지 않는다.
