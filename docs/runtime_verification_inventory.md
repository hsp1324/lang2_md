...
Production CE27 verifies the Scenario 14 route map, all 19 text-bearing scrolling frames of 성검 랑그릿사, and the preparation endpoint. Direct Japanese-ROM rendering confirms that ancient Baldia sleeps at the bottom of a deep lake, Langrisser rests in the castle depths, and the Blue Dragon Knights seek the same objective. Explicit source rewrapping removed isolated 다스렸던, 드러낸, and 전설의 lines while retaining the existing 렸 glyph so no global direct-string token IDs moved. The canonical 발디아 spelling and every event remain natural Korean without Japanese residue, broken glyphs, clipping, or abnormal spacing. Earlier current captures verified all seven selectable commander/class rows, arrangement, the complete opening path, condition rows, and the first turn back to a valid command menu. Later turns, completion, and branches remain pending; battle_ui remains covered by the shared probe.
