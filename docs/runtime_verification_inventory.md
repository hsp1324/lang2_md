...
Current hard candidate FBE2 additionally verifies the hidden Rune Stone and
all-factions-cheat report. The one-based `(5,4)` well trigger
`0x18D768..0x18D778`, its handler `0x18D8D8..0x18D8F0`, and all four
Aaron/resident fixed records are byte-identical in the Japanese ROM, normal
candidate, and hard candidate. Stock resident movement can obstruct the well;
this is source behavior, not a hard-mode coordinate regression. Entering the
original all-factions command with 50 ms holds/gaps and selecting an enemy
commander opened its Korean `이동 / 공격` menu in
`captures/run/hard_fbe2_s06_all_factions_enemy_command.png`. Hard mode therefore
does not disable the command. Full hashes and the rejected coordinate-change
rationale are recorded in `localization/scenario6_runestone_runtime.json`.
