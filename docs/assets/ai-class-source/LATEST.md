...
색이 아니라 대상의 원본 클래스 팔레트를 사용한다. 리아나 하이로드는
금발과 얼굴의 갈색 픽셀을 보호하고, 마스크 바깥 갑옷과 망토의
갈색·빨간색 주면을 왕청색으로, 어두운 주름을 남청색으로 바꾼다.
아론 로드는 회색
...
내부 클래스 레코드는 별도다. 일반 NPC 하이로드 `99`와 같은 원본
스프라이트 `0x1C`에서 흰색·회색 갑옷 면만 밝은 연두색으로 바꾸고 가장
깊은 음영은 짙은 초록색으로 남긴다. 오른쪽 흰 검과 파랑·하늘색 방패,
금색 테두리, 실루엣과 모든 픽셀 위치는 원본을 유지한다.
