...
- `tools/magic_flow_inventory.py`: 마법 22종의 원본 소유 클래스, 자연 전직 경로, 요구 레벨, MP 비용을 전부 연결합니다. 21종은 자연 획득 가능한 대표 경로를 기록하고, `텔레포트`는 플레이어 트리와 고정 시나리오에 없는 `에이전트(25)` 전용 원본 예외로 고정합니다. 공용 학습·명령·목록·선택·실행 루틴의 일본판 동등성과 22종 선택/실행 화면·GST를 검증해 `localization/magic_flow_inventory.json`과 `docs/magic_flow_inventory.md`를 생성합니다.
...
- `tools/jp_short_inline_byte_inventory.py`: 위 스캔이 의도적으로 제외한 의미 바이트 1~2개의 짧은 `FF` 종료 후보 6,612개를 영역별로 집계합니다. 일본어 16x16 폰트/클래스/스프라이트/아이템/이름/그래픽/시스템/엔딩/시나리오·텍스트/UI·압축 리소스 뱅크와 실행 코드 블록을 포함한 6,612개 전체를 구조와 16비트 문맥, 직접·PC 상대 참조로 대조해 6,611개 명령·비트맵·포인터 경계·글리프 워드·그래픽·레이아웃·압축 페이로드 오탐과 시나리오 설명 병력 레벨 접두사 `L-`로 분류합니다. `0x000270..0x005DD3`의 127개 중 123개는 다섯 연속 68000 명령 스트림 안에 있고, 네 개는 38개 포인터가 소유하는 16비트 워드 레코드의 `0x004A` 하위 바이트임을 고정합니다. `0x008000..0x008F4D`의 41개 중 40개는 부팅·인터럽트 코드, 한 개는 초기화 설정표이며 사이의 네 데이터 표와 앞의 8,748바이트 `FF` 패딩을 분리해 검증합니다. 이어지는 `0x008F4E..0x0099EF`의 67개는 742개 연속 68000 명령 안에 있고, 뒤의 `0x0099F0..0x0099F9` 10바이트는 다음 루틴이 사용하는 PC 인덱스 디스패치 오프셋 5개이며 후보가 없음을 별도로 고정합니다. `0x0099FA..0x00D47D`의 174개는 3,980개 연속 68000 명령 안에 있고, 뒤의 33바이트 메뉴 문자열 `LOADSAVECONTINUESCENARIONOTHING !`를 코드 경계로 고정합니다. 후보를 가리키는 유일한 4바이트 창은 홀수 주소 `0x00A0CF`를 향해 코드 진입점이 될 수 없습니다. 메뉴 문자열의 마지막 `!`와 첫 opcode 바이트를 공유하는 `0x00D49E..0x00FE27`에는 후보 85개를 포함한 두 연속 코드 스트림과 후보가 없는 32바이트 입력 패턴 표 및 7개 포인터 소유 레이아웃 표가 있습니다. 직접 참조 두 건은 정확한 명령 시작점 `0x00DA2A`, `0x00FA28`을 가리킵니다. `0x00FE28..0x011FC7`의 후보 102개는 다섯 연속 68000 명령 스트림 안에 있고, 사이의 자리값·10개 포인터 소유 수치 레코드·방향 패턴·14개 레이아웃 레코드·설정 표에는 후보가 없습니다. 35개 직접 참조 창이 가리키는 네 후보 주소는 모두 `DBRA` 또는 `MOVE` 명령 내부의 홀수 주소이므로 코드 진입점이 아닙니다. 이어지는 `0x011FC8..0x012EBD`의 후보 24개도 853개 연속 명령 안에 있고, 마지막 `RTS`까지 정확히 끝나며 후보를 가리키는 직접·PC 상대 참조가 없음을 검증합니다. `0x012EBE..0x017385`의 후보 189개는 4,317개 연속 명령 안에 있고, 뒤의 후보 없는 9워드 패턴표는 `LEA $017374`로 직접 참조됩니다. 후보 참조 27건은 홀수 명령 내부 주소 두 곳과 실제 `TST.B` 진입점 한 곳을 가리킵니다. `0x017386..0x018091`의 후보 35개는 두 연속 코드 스트림 안에 있고, 사이의 PC 인덱스 4바이트 표와 원본 `ｽﾃﾙ ｱｲﾃﾑ ｾﾝﾀｸ` 및 `PAGE` 표시 레코드는 후보가 없는 별도 데이터입니다. `0x018092..0x018C37`의 후보 18개는 정확한 612개 명령 스트림 안에 있고, 뒤의 후보 없는 54포인터 함수표는 `0x018B3E`와 `0x01A63E`의 두 `LEA` 피연산자가 참조합니다. 후보 참조 한 건은 홀수 `DBRA` 명령 내부 바이트를 가리킵니다. `0x018C38..0x01A86F`의 후보 42개는 두 코드 스트림과 직접 참조되는 22바이트 수치표 및 271워드 부호 있는 삼각함수 표에 속하며, 코드·데이터 경계와 전체 해시를 고정합니다. `0x01A870..0x01DC59`의 후보 69개는 정확한 3,305개 연속 명령 스트림 안에 있고, 후보 참조 한 건은 홀수 `DBRA` 명령 내부 바이트를 가리킵니다. `0x01DC5A..0x01E05B`의 후보 10개는 251개 명령 스트림과 직접 참조되는 자리값·카운트·포인터·전송 설명자 표에 속하며, 설명자 카운트의 하위 바이트 `R/P/T/P`는 문자열이 아님을 고정합니다. `0x01E05C..0x01FFFF`의 마지막 후보 140개도 1,543개 연속 명령 안에 있고, 구간 시작을 직접 호출하는 `JSR` 6건과 전체 해시를 검증합니다. `0x020000..0x02A19B`와 `0x02A1B0..0x02BFC7`의 228개는 두 연속 68000 명령 스트림 안에 있고, 사이의 `0x02A19C..0x02A1AF` 20바이트 수치 표에는 후보가 없음을 별도로 고정합니다. 일치하는 7개 직접 참조 창은 모두 홀수 후보 주소를 가리켜 68000 코드 진입점이 될 수 없습니다. `0x02BFC8..0x02FFFF`의 108개는 3,451개 연속 명령 안에 있고 37개 직접 참조가 네 코드 진입점을 가리킴을 검증합니다. `0x030000..0x03109F`의 21개는 699개 연속 68000 명령의 opcode/immediate와 `FFFFFxxx` 절대주소 피연산자가 맞닿아 생긴 오탐으로 명령 주소·원시 바이트를 고정합니다. `0x040000..0x04FFFF`의 1,477개는 1,024글자×64바이트 고정 폰트 뱅크의 전체 SHA와 후보 manifest SHA로 잠급니다. `0x0B0000..0x17FFFF`의 3,254개는 429개 압축 리소스의 포인터 표와 type 1/2/3 실제 인코딩 끝을 파싱해 모두 압축 페이로드 내부이고 포인터 표·정렬/꼬리 패딩에는 후보가 없음을 검증합니다. `0x050000..0x07FFFF`에서 포인터처럼 보인 다섯 개의 4바이트 창도 수치/그래픽 인덱스 행 내부 조합 두 건과 68000 명령/피연산자 경계 조합 세 건으로 판독해 실제 포인터에서 제외합니다. 이로써 짧은 후보 6,612개의 소유권 분류를 모두 완료했으며, 구조 오탐 6,611개와 실제 시나리오 레벨 접두사 `L-` 한 건을 구분합니다.
...
- `tools/release_acceptance.py`: 현재 한국어 ROM의 체크섬·SHA-256, 31×8 실기 행렬, 전체 텍스트 페이지, UI·이름·아이템·클래스·전직·마법·엔딩, 잔존 문자열 후보, 압축 리소스 소유권, 마지막 빌드 델타와 편집기 무변경 왕복을 Goal 8개 완료 조건으로 묶어 검사합니다. `--write`는 `localization/release_acceptance.json`과 `docs/release_acceptance.md`를 생성하며, 하드 모드는 별도 밸런스 합의 전 미착수 상태로 고정합니다.
...
- `tools/hard_mode_playtest.py`: 정확한 하드 후보 SHA-256에 시나리오별 실제 클리어, 체감 난이도, 턴 수, 재시도, 문제와 증거를 묶어 기록합니다. 후보가 바뀌면 이전 결과를 자동으로 완료 수에서 제외하며, `--require-complete`는 31/31 실제 클리어 전까지 실패합니다.
...
- `tools/build_scenario5_escape_probe_rom.py`: 기본 모드는 시나리오 5의 `22턴 내 북쪽 도착`을 원본 이벤트로 재현하기 위해 첫 엘윈 배치의 Y만 `50→1`로 바꿉니다. `--enemy-annihilation`은 원본 적 9명의 정체·클래스·레벨·좌표·이벤트를 보존하고 AT/DF·용병만 제한한 뒤, Start 진입 때 런타임 적 8명을 제거하고 남은 원본 늑대인간의 HP만 1로 낮춥니다. `--protagonist-death`는 엘윈 런타임 그룹만 패배 처리하고, `--timeout`은 Start에서 검증된 턴 카운터 `$FFFFA5F1`을 마지막 허용 턴 22로 한 번만 올립니다. `--timeout-alternate`는 스코트가 없을 때의 원본 일반병 대사 본문을 별도로 검증하는 진단 브리지입니다. `--turn-event {16,20,22}`는 더 이른 세이브에서 카운터를 목표 턴 직전까지만 올려 원본 예약 이벤트를 재현하고, `--turn-event-alternate`는 20턴 스코트 대사의 원본 일반병 대체 본문을 검증합니다. 두 승리, 주인공 사망, 스코트/일반병 제한 턴 초과 분기와 16·20·22턴 예약 이벤트를 모두 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario6_clear_probe_rom.py`: 기본 모드는 시나리오 6의 아군·NPC·민간인·이벤트를 보존하고 적 고정 레코드 4~12의 AT/DF와 용병만 제한하며, 가시 적을 원본 플레이어 배치 근처로 옮겨 민간인 전원 생존 클리어를 재현합니다. `--civilian-loss`는 시민 고정 레코드를 바꾸지 않고 Start 진입 때 선택한 `--lost-civilian-record 1..3`을 런타임 손실 상태로 만들며, 각 주민 사망 반응·아뮬렛 미지급·전과보고·7장 저장을 빠르게 확인합니다. `--protagonist-death`는 모든 시나리오 고정 레코드를 보존하고 엘윈 런타임 상태만 패배로 바꿉니다. `--turn-event {3,4,5,7}`은 원본 배치·고정 레코드·예약 이벤트를 보존한 채 Start에서 턴 카운터를 필요한 값까지만 올리고, 무행동 진행 중 아군·아론·주민이 죽지 않도록 해당 런타임 그룹의 DF만 진단용으로 보호합니다. 턴 7의 `--turn-event-branch`는 예약 테이블 대상 4바이트만 원본 핸들러 내부의 지원 도착·모건 대체 반응·지각 본문으로 연결합니다. 전원 생존, 여섯 가지 부분 손실, 시민 전멸, 주인공 사망 GAME OVER와 3·4·5·7턴의 모든 대사 분기를 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario7_clear_probe_rom.py`: 기본 모드는 시나리오 7의 주민·숨은 키스·나머지 적·이벤트를 보존하고 기남 한 명의 AT/DF·용병·좌표만 제한합니다. `--civilian-loss`는 주민 고정 레코드를 바꾸지 않고 Start 진입 때 선택한 `--lost-civilian-record 0..2`를 런타임 손실 상태로 만들며, `--protagonist-death`는 모든 고정 레코드를 보존하고 엘윈 런타임 상태만 패배로 바꿉니다. `--turn-event {3,6}`은 모든 배치·고정 레코드·원본 이벤트를 보존하고 아군·주민·숨은 키스 런타임 그룹의 DF만 진단용으로 보호한 뒤 턴 카운터를 필요한 값까지만 올립니다. 6턴의 `--turn-event-branch dialogue`는 실기 발동이 확인된 6턴 진입 슬롯의 대상 4바이트만 원본 6턴 종료 핸들러 내부의 조건부 대사 본문으로 연결하며, 원래 조건이 충족됐다고 주장하는 용도가 아닙니다. 주민 전원 생존 보상인 미라쥬로브·룬스톤, 일곱 가지 주민 손실 조합의 전용 반응과 보상 생략, 주민 전멸·주인공 사망 GAME OVER와 3·5·6턴 예약 이벤트 및 모든 대사 본문을 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario8_clear_probe_rom.py`: 기본 모드는 시나리오 8의 크레이머 한 명만 원본 엘윈 배치 바로 위로 옮기고 AT/DF와 용병을 제한합니다. 첫 정상 공격은 원본 보스 생존 처리로 HP 1을 남기므로 턴 종료 뒤 두 번째 정상 공격으로 격파해야 합니다. `--protagonist-death`는 원본 고정 레코드를 모두 보존하고 엘윈 런타임 그룹만 패배 처리하며, `--timeout`은 세 차례 GST 비교로 확정한 턴 카운터 `$FFFFA5F1`을 마지막 허용 턴 23으로 설정합니다. `--turn-event {2,12,18,21,23}`과 `--turn-event-sequence`는 원본 배치·고정 레코드·예약 이벤트를 보존하고 일곱 플레이어 그룹의 DF만 보호한 뒤 카운터를 필요한 값까지만 올립니다. `--turn-23-no-scott`는 일본판 명단 순서에서 스코트가 런타임 그룹 2임을 원본 바이트로 검증하고 그 그룹만 부재 처리해 23턴의 일반병 대체 대사를 재현합니다. 정상 클리어·보스 생존·주인공 사망·제한 턴 초과와 1·2·12·18·21·23턴의 모든 예약 대사 및 23턴 스코트/일반병 변형을 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
...
- `tools/build_scenario9_clear_probe_rom.py`: 기본 모드는 시나리오 9의 레아드 한 명만 원본 엘윈 배치 바로 위로 옮기고 AT/DF와 용병을 제한합니다. `--npc-annihilation`은 원본 고정 레코드를 모두 보존하고 NPC 런타임 그룹 `7..9`만 패배 처리하며, `--protagonist-death`는 엘윈 그룹만 패배 처리합니다. `--turn-event`는 모든 배치·고정 레코드·예약 이벤트·핸들러를 보존한 채 아군/NPC 그룹만 보호하고 턴 카운터를 1로 올려 턴 1 종료와 턴 2 진입 이벤트를 재현합니다. 레아드 격파·NPC 전멸·주인공 사망·모든 예약 턴 이벤트, 정상 경로의 전과보고·10장 저장과 진입까지 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario10_clear_probe_rom.py`: 기본 모드는 시나리오 10의 원본 배치·이벤트를 보존하고 3턴에 출현하는 마물 10명의 AT/DF·용병만 제한합니다. `--protagonist-death`는 다섯 플레이어 배치와 해적·레스터·마물의 13개 고정 레코드를 모두 보존하고 엘윈 런타임 그룹만 패배 처리합니다. 마물 전멸 정상 클리어·전과보고·11장 저장과 주인공 사망 전용 대사·GAME OVER를 실기 검증했으며 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario11_clear_probe_rom.py`: 시나리오 11의 일본판 헤더·배치표·11개 고정 레코드를 검증하고 적 능력치·용병을 제한합니다. `--safe-clear-layout --safe-jessica` 진단 조합은 이벤트를 보존한 채 전투 진영과 제시카만 화재 밖으로 옮겨 9턴 화재·숨은 증원·전과보고·12장 저장과 진입까지 실기로 검증했습니다. `--protagonist-death`와 `--jessica-death`는 모든 고정 데이터를 보존하고 해당 런타임 그룹만 패배 처리하며 두 GAME OVER 분기도 실기 검증했습니다. 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario12_clear_probe_rom.py`: 시나리오 12의 일본판 헤더·배치표·11개 수호자 레코드와 세 개의 맵 트리거를 검증하고 적 AT/DF·용병만 제한합니다. 선택적인 압축 배치는 클리어 경로 진단용이며 이벤트 카메라에서 맵 스프라이트가 HUD 위로 보일 수 있으므로 UI 증거에는 원본 플레이어 좌표를 보존하는 기본 모드를 사용합니다. 압축 체크섬 `8B33`에서는 마지막 `리빙아머`를 정상 공격으로 격파한 뒤 다크로드 후속 대사, 전과보고, 13장 저장·진입까지 확인했습니다. `--protagonist-death`는 원본 배치와 고정 레코드를 보존하고 런타임 엘윈만 사망 처리하며, 실기 검증 당시 체크섬 `E3EE`에서 `크윽! 여기까지인가…`와 GAME OVER를 확인했습니다.
- `tools/build_scenario13_clear_probe_rom.py`: 시나리오 13의 일본판 헤더·7개 플레이어 배치·13개 적 레코드를 검증합니다. 기본 모드는 적 AT/DF·용병만 제한하고 조름만 원본 엘윈 바로 아래로 옮겨 턴 이벤트를 확인합니다. `--completion-layout`은 비배포 진단용으로 일반 적을 숨기고 발가스 출현 구역에 아군을 모으며, Start 진입 때 런타임 이름 ID가 실제 발가스인지 검사한 뒤 HP만 1로 낮춥니다. checksum `0CCD`에서 정상 `매직애로우`로 발가스를 격파하고 전과보고, 14장 저장·경로·진입까지 확인했습니다. `--protagonist-death`는 모든 배치와 고정 레코드를 보존하고 런타임 엘윈만 사망 처리하며, 실기 검증 당시 checksum `E3EE`에서 첫 턴 전술 대사 뒤 `젠장! 끝인가…`와 GAME OVER를 확인했습니다. 진단 모드의 배치·발가스 클래스·HP는 원본이나 편집기 기본값의 근거로 사용하지 않습니다.
- `tools/build_scenario14_clear_probe_rom.py`: 시나리오 14의 일본판 헤더·7명 배치표·11개 적 레코드와 엘윈·쉐리·제시카 및 레온의 원본 랑그릿사 도달 트리거를 검증하고 적 AT/DF·용병만 제한합니다. 기본 모드는 모든 배치를 보존하며, `--completion-layout`은 엘윈만 원본 목표 `(16,6)` 바로 아래 `(16,7)`로 옮기는 비배포 진단입니다. checksum `B658`에서 정상 이동으로 랑그릿사를 얻고 레온 퇴각·아군 후속·전과보고·15장 저장·진입까지 확인했습니다. `--protagonist-death`는 런타임 엘윈만 패배 처리하고, `--leon-langrisser`는 일본판의 숨은 레온 레코드만 정확한 목표 `(16,6)`에 활성화합니다. 현재 생산 ROM `00DC` 기준 후자는 checksum `90E3`이며 랑그릿사 획득·철수·GAME OVER 전 과정을 확인했습니다. 진단 배치와 제한 수치는 편집기 기본값의 근거로 사용하지 않습니다.
- `tools/build_scenario15_clear_probe_rom.py`: 시나리오 15의 일본판 헤더·7명 배치표·12개 고정 레코드와 남쪽 조우·엘윈 탈출·패배 트리거를 검증하고 적 레코드 1~11의 AT/DF·용병만 제한합니다. 기본 모드는 아군 스코트와 모든 원본 좌표, 이멜다 및 숨은 스큐라·라나·와이번을 보존합니다. `--completion-layout`은 엘윈만 원본 탈출 범위 `X 1..46 / Y 21..22` 바로 위 `(3,20)`으로 옮기며, checksum `35A3`에서 정상 이동으로 라나·마물 이벤트, 이멜다 퇴각, 스코트 합류, 전과보고, 16장 저장·진입까지 확인했습니다. `--protagonist-death`는 모든 배치와 고정 레코드를 보존하고 Start 진입 때 런타임 엘윈만 패배 처리합니다. 생산 ROM `00DC` 기반 checksum `F7AB`에서 정상 턴 종료 뒤 원본 즉시 GAME OVER 처리까지 확인했습니다. `--turn-event {2,3,6,7,8}`은 검증 당시 생산 ROM `653C`에서 모든 배치·고정 레코드·예약 테이블·핸들러를 보존하고 플레이어 7명과 스코트만 DF 99로 보호한 뒤 공유 턴 카운터를 필요한 값까지만 올리며 checksum `9303/9307/930B/930D/930F`를 생성했습니다. 실기 검증은 `9303`의 1·2턴, `9307`의 3턴, `930B`에서 연속 진행한 6·7·8턴을 사용했습니다. `--turn-event-branch imperial-soldier` checksum `9331`은 3턴 슬롯의 대상 4바이트만 원본 제국병 대체 본문으로 연결합니다. 현행 `E1AF` 기반 재생성값은 각각 `0F76/0F7A/0F7E/0F80/0F82`, 대체 분기는 `0FA4`입니다. `--enemy-defeat-record 1..7`은 선택한 가시 적 지휘관 하나만 엘윈 아래 `(3,3)`으로 옮기고 AT/DF·용병을 제한하며 다른 레코드와 모든 이벤트를 보존합니다. v1.3.9 Normal 후보의 record 1 checksum `17F7`에서 일반 공격 격파, 다음 적 페이즈의 18개 대사, 행동 지휘관의 돌격 대사와 다음 플레이어 턴 복귀를 실기로 확인했습니다. 6턴 공용 핸들러의 후반 완료 본문은 기존 정상 탈출 검증이 담당합니다.
- `tools/build_scenario16_clear_probe_rom.py`: 시나리오 16의 일본판 헤더·8명 배치표·10개 적 레코드와 성문·숨은 글레이프니르·패배 트리거를 검증하고 AT/DF·용병만 제한합니다. 레온·레아드와 숨은 라나·고스트의 정체·클래스·레벨·좌표 및 모든 이벤트를 보존합니다. `--completion-layout`은 엘윈만 원본 성문 범위 `X 12..14 / Y 1..5` 바로 아래 `(13,6)`로 옮기며, checksum `8D76`에서 정상 이동으로 레온 퇴각·아군 후속·전과보고 `6000P`·17장 저장·진입까지 확인했습니다. `--protagonist-death`는 모든 배치와 고정 레코드를 보존하고 런타임 엘윈만 패배 처리합니다. 현재 생산 ROM `00DC` 기준 checksum `F7AB`에서 `으윽… 여기까지인가…`와 GAME OVER를 실기 확인했습니다. `--turn-event {3,8}`은 모든 배치·고정 레코드·예약 턴 표·원본 핸들러를 보존하고 아군 8개 런타임 그룹만 DF 99로 보호한 뒤 턴 카운터만 올립니다. 검증 당시 생산 ROM `653C` 기준 checksum `9307`/`9311`에서 상태 전용 3턴 이벤트와 라나·레온·엘윈의 8턴 대사 5페이지, TURN 9 명령창 복귀를 확인했습니다. 현행 `E1AF` 기반 재생성값은 `0F7A`/`0F84`입니다.
- `tools/build_scenario17_clear_probe_rom.py`: 시나리오 17의 일본판 헤더·8명 배치표·11개 적 레코드와 주인공 패배 이벤트를 검증하고 AT/DF·용병만 제한합니다. 베른하르트·보젤과 숨은 매직나이트의 정체·클래스·레벨·좌표 및 모든 이벤트를 보존합니다. `--completion-layout`은 베른하르트를 원본 옥좌 `(15,4)`에 둔 채 엘윈만 바로 아래 `(15,5)`로 옮기며, checksum `B17C`에서 실제 공격 두 번으로 베른하르트를 격파하고 보젤 퇴각·후속 대사·레벨업·클래스 체인지·18장 저장·진입까지 확인했습니다. `--protagonist-death`는 모든 배치와 고정 레코드를 보존하고 런타임 엘윈만 패배 처리합니다. 현재 생산 ROM `00DC` 기준 checksum `F7AB`에서 `여기까지 와서 죽는 건가…`와 GAME OVER를 실기 확인했습니다.
- `tools/build_scenario18_clear_probe_rom.py`: 시나리오 18의 일본판 헤더·8명 배치표·주민 2명·적군 9명을 검증합니다. 주민 레코드는 통째로 보존하고 적군 AT/DF·용병만 제한합니다. `--completion-layout`은 그레이트드래곤을 원본 `(35,4)`에 둔 채 엘윈만 바로 아래 `(35,5)`로 옮깁니다. checksum `17F2`에서 실제 전투로 드래곤을 격파하고 주민 생존 대사·클래스 체인지·19장 저장·진입까지 확인했습니다. 기록 당시 기본 파생 체크섬은 `17DF`입니다.
- `tools/build_scenario19_clear_probe_rom.py`: 시나리오 19의 일본판 헤더·8명 배치표·적군 10명을 검증하고 AT/DF·용병만 제한합니다. 이멜다와 숨은 레아드·제국 증원의 정체·클래스·레벨·좌표 및 모든 이벤트를 보존합니다. `--completion-layout`은 이멜다를 원본 `(37,23)`에 둔 채 엘윈만 바로 위 `(37,22)`로 옮깁니다. 과거 checksum `2829`와 격리 저장 슬롯 AT/DF 99에서 이멜다 격파·레아드 증원·출항·20장 저장과 진입을 확인했습니다. 현재 생산 ROM `E1AF` 기준 기본/완료 배치는 `2B30`/`2B60`입니다. `--protagonist-death`는 원본 배치·고정 레코드·`0x1A5E60` 트리거와 `0x1A6144` 핸들러를 보존하고 런타임 엘윈만 사망 처리하며, 현재 체크섬은 `D87E`입니다. `--turn-event {2,13,18,19,21,23}`은 원본 배치·고정 레코드·예약 테이블·핸들러를 모두 보존하고 런타임 아군 8명만 보호한 채 지정 턴으로 진행합니다. 현재 체크섬 `0F76`/`0F8E`/`0F98`/`0F9A`/`0F9E`/`0FA2`에서 모든 예약 턴 대사, 다음 턴 복귀, 턴 23의 원본 시간초과 GAME OVER를 실기로 확인했습니다.
- `tools/build_scenario20_clear_probe_rom.py`: 시나리오 20의 일본판 헤더·8명 배치표·적군 10명과 원본 이벤트 트리거·핸들러를 검증하고 기본 모드에서는 AT/DF·용병만 제한합니다. 파이어스와 숨은 와이번·크라켄의 정체·클래스·레벨·좌표를 보존합니다. `--completion-layout`은 파이어스의 원본 레코드 인덱스 5를 유지한 채 다른 9개 레코드를 숨기고 엘윈만 바로 위 `(22,22)`로 옮기며, checksum `D2F9`에서 파이어스 격파·전과보고·21장 저장과 진입을 확인했습니다. `--protagonist-death` checksum `AAC1`은 원본 고정 데이터를 보존하고 런타임 엘윈만 패배 처리해 GAME OVER를 확인했습니다. `--kraken-event` checksum `AF2A`와 `--conditional-dialogues` checksum `AF30`은 각각 원본 크라켄 출현 이벤트와 도렌 복수·리아나 위협 대사를 턴 3·4·5에 재현하며, 모든 페이지와 명령 복귀를 실기 검증했습니다. 세 진단 모드는 상호 배타적이고 생성 ROM은 커밋하지 않습니다.
- `tools/build_scenario21_clear_probe_rom.py`: 시나리오 21의 일본판 헤더·8명 배치표·적군 11명과 주인공 패배 트리거를 검증합니다. 기본/완료 모드는 라나와 숨은 크라켄·제국 아크메이지의 정체·클래스·레벨·좌표 및 모든 이벤트를 보존한 채 적 AT/DF·용병만 제한합니다. `--protagonist-death`는 모든 배치·고정 레코드·`0x1A958C` 트리거와 `0x1A97BE` 핸들러를 보존하고 런타임 엘윈만 패배 처리하며, checksum `AAC1`에서 `젠장… 막 벨제리아에 왔는데…`와 GAME OVER를 실기 확인했습니다.
- `tools/build_scenario22_clear_probe_rom.py`: 시나리오 22의 일본판 헤더·8명 배치표·12개 고정 레코드와 엘윈·제시카 패배 이벤트를 검증합니다. 기본/완료 모드는 아군 리아나를 통째로 보존하고, 이벤트 전환용 특수 진영과 숨은 베른하르트를 포함한 전투 레코드 1~11의 AT/DF·용병만 제한합니다. `--protagonist-death`와 `--jessica-death`는 모든 배치·고정 레코드·전투 수치·원본 이벤트를 보존하고 런타임의 해당 지휘관만 패배 처리합니다. checksum `AAC1`/`B181`에서 `큭… 여기까지인가…`/`아아… 하늘이 푸르군…`과 GAME OVER를 실기 확인했습니다.
...
- `tools/build_scenario24_clear_probe_rom.py`: 시나리오 24의 일본판 헤더·9명 배치표·11개 고정 레코드와 주인공 패배 이벤트를 검증합니다. 기본 모드는 특수 진영 베른하르트를 통째로 보존하고 데몬로드·리치·케르베로스·뱀파이어로드 적군 10명의 AT/DF·용병만 제한합니다. `--completion-target-only`는 이벤트가 되살리는 런타임 그룹을 Start에서 숨김·격파 처리하고 원본 뱀파이어로드만 엘윈 옆 공격 대상으로 남겨 완료 대사·전과보고·25장 저장을 검사합니다. `--protagonist-death`는 모든 배치·고정 레코드를 보존하고 런타임 엘윈만 패배 처리해 원본 GAME OVER 경로를 검사하는 비배포 진단입니다.
- `tools/build_scenario25_clear_probe_rom.py`: 시나리오 25의 일본판 헤더·9명 배치표·12개 고정 레코드를 검증합니다. 기본 모드는 아군 제시카를 통째로 보존하고 레온·레아드·에그베르트 및 숨은 드래곤로드를 포함한 적군 11명의 AT/DF·용병만 제한합니다. `--completion-target-only`는 개막 이벤트 뒤 레온만 원본 좌표에 남기고 엘윈을 바로 아래에 배치해 정상 전투·전과보고·26장 저장과 진입을 확인하는 비배포 진단이며, `--protagonist-death`는 모든 배치·고정 레코드를 보존한 채 Start에서 런타임 엘윈 그룹만 패배 처리하여 원본 패배 이벤트와 GAME OVER를 검증합니다.
- `tools/build_scenario26_clear_probe_rom.py`: 시나리오 26의 일본판 헤더·10명 배치표·적군 10명을 검증하고 AT/DF·용병만 제한합니다. 아크메이지·위저드·세인트·나이트마스터와 에그베르트의 정체·클래스·레벨·좌표 및 이벤트를 보존합니다. `--battle-ui-target-only`는 원본 아크메이지만 일반 전투 대상으로 남겨 전투 수치·지형·이름·클래스 렌더러를 검사하고, `--completion-target-only`는 개막 뒤 원본 에그베르트만 HP 1로 남겨 보상·아이템 폐기·후속 대사·전과보고·27장 저장까지 검사합니다. `--protagonist-death`는 원본 패배 이벤트를 보존한 채 Start 진입 시 런타임 주인공 그룹만 패배 처리합니다. 세 모드는 원본 고정 레코드를 수정하지 않는 상호 배타적 비배포 진단입니다.
...
- `tools/build_scenario28_clear_probe_rom.py`: 비밀 시나리오 X1의 일본판 헤더·7명 배치표·빌더 적군 9명을 검증하고 기본 모드에서는 AT/DF·용병만 제한합니다. `--completion-target-only`는 준비 배치표를 원본 그대로 두고 바란만 첫 엘윈 배치 바로 위로 옮기며, Start에서 다른 빌더 런타임 그룹을 제거하고 바란 HP만 1로 낮춥니다. checksum `31F2`에서 정상 공격으로 바란을 격파해 전체 후속 대사·전과보고·13장 저장·진군루트·13장 복귀까지 확인했습니다. `--protagonist-death`는 모든 배치·고정 레코드·원본 패배 이벤트를 보존하고 런타임 엘윈만 패배 처리하며, checksum `5A95`에서 바란 턴 대사·엘윈 패배 대사·GAME OVER를 실기 확인했습니다.
- `tools/build_scenario29_clear_probe_rom.py`: 비밀 시나리오 X2의 일본판 헤더·8명 배치표·적군 9명을 검증하고 기본 모드에서는 AT/DF·용병만 제한합니다. `--completion-target-only`는 준비 배치표를 원본 그대로 두고 폴거만 첫 엘윈 배치 바로 위로 옮기며, Start에서 다른 적 런타임 그룹을 제거하고 폴거 HP만 1로 낮춥니다. checksum `A042`에서 정상 공격으로 폴거를 격파해 전체 후속 대사·전과보고·20장 저장·진군루트·20장 복귀까지 확인했습니다. `--protagonist-death`는 모든 배치·고정 레코드·원본 패배 이벤트를 보존하고 런타임 엘윈만 패배 처리하며, checksum `5A95`에서 세이갈·폴거 턴 대사·엘윈 패배 대사·GAME OVER를 실기 확인했습니다.
- `tools/build_scenario30_clear_probe_rom.py`: 비밀 시나리오 X3의 일본판 헤더·9명 배치표·적군 11명과 주인공 패배 이벤트를 검증하고 기본 모드에서는 AT/DF·용병만 제한합니다. `--completion-target-only`는 일본판 미나 좌표와 메이지→세인트 변신 이벤트를 그대로 두고 첫 엘윈 배치만 바로 아래 `(15,7)`로 옮기며, Start에서 다른 런타임 적을 제거하고 현재 미나 HP만 1로 낮춥니다. checksum `8386`에서 두 형태를 정상 공격으로 격파해 완드·독보우·미라쥬로브와 소지품 폐기, 후속 대사·전과보고·23장 저장·진군루트·23장 복귀까지 확인했습니다. 현행 ROM 첫 턴 검증에서 원본 `0x1B76A2` 패배 트리거의 `젠장! 왜 이런 곳에…`와 GAME OVER도 확인했습니다.
...
빌드합니다. 현재 일반판은 타이틀에 `번역:1.2.1`, 하드판은 아래쪽
왼쪽과 오른쪽에 `번역:1.2.1`, `하드:1.2.1`을 나눠 표시합니다.
일반판 배포 파일명은
`Langrisser II (Korean v1.2.1).md`이며 ROM 영문 게임명 헤더에도
`LANGRISSER II KOREAN T1.2.1 BY HSP1324`가 기록됩니다. 일본어 게임명,
제품 코드와 SRAM 헤더는 바꾸지 않습니다.

```bash
python3 scripts/build_korean_jp_probe.py \
  --rom-profile normal \
  --out "roms/builds/Langrisser II (Korean v1.2.1).md"
...
```text
roms/builds/Langrisser II (Korean Hard T1.2.1 B1.2.1).md
MD checksum: C9BA
SHA-256: 7c955c09069a56260702104a3587b8e362eede5d4c7fc55bbad259e616f2f123
```
...
단일 실행 패처의 동작 규격은
[`player_patch_distribution.md`](player_patch_distribution.md)에
...
[`save_preserving_rom_updates.md`](save_preserving_rom_updates.md)에
정리했습니다.
...
브라우저에서 `http://127.0.0.1:8765`를 엽니다. 탭에서 31개 시나리오의 클래스·LV·AT·DF·용병 6칸, 37개 아이템의 가격·효과 4칸, 10명 지휘관의 클래스 체인지 경로와 전직 시 새로 해금할 용병을 수정할 수 있습니다. 시나리오와 클래스 트리에는 ROM에서 직접 추출한 클래스/용병 그림이 표시되고, 아이템 표에는 실기 캡처로 검증한 원본 아이콘이 표시됩니다. 용병 선택기는 157개 클래스 ID의 그림과 한국어·일본어 이름을 함께 보여줍니다. `AI 클래스`에서는 AI 원화나 사용자가 불러온 다른 이미지를 참고하면서 상위 클래스의 최종 16×16 도트를 직접 편집하고 저장할 수 있습니다. `테스트 체인지`와 AI 디자인 PNG는 아직 `편집 ROM 빌드` 입력에 포함되지 않습니다. `편집 ROM 빌드`는 원본을 덮어쓰지 않고 다음 파일을 만듭니다.
...
아이템 효과 표는 `0x060530`이며 ID마다 `(효과 종류, 값)` 4쌍, 총 8바이트입니다. 가격 표는 `0x0A1D32`이며 ROM 값에 10을 곱한 값이 게임에서 P로 표시됩니다. 경험치 2배, 특수 사거리, 룬스톤, MP 2배처럼 별도 코드에 하드코딩된 효과는 아직 읽기 전용으로 표시합니다. 클래스 체인지 포인터 표는 `0x08253A`이며 엘윈부터 제시카까지 10명의 10개 분기 레코드를 실제 최대 5단계 트리로 표시합니다. 클래스 레코드 표 `0x05EDDC`의 `+0x1A/+0x1B`는 전직 시 누적 해금할 MD판 용병 2종입니다. 자세한 구조와 제한은 `docs/editor_data_model.md`에 기록했습니다.
...
고정 배치 레코드의 클래스 ID만 바꾸면 화면상의 클래스명과 능력치는 바뀌지만 마법·소환 명령은 부여되지 않습니다. REV00 시나리오 1에서 레아드를 `서머너`로 바꾼 실기 검증에서도 `이동/공격/치료/명령`만 남았습니다. 자연 성장에서는 레벨업 처리 `0x014946..0x01498A`가 새 클래스의 능력 ID와 요구 레벨을 읽어 런타임 `+0x50` 플래그를 설정합니다. 헤인은 `위저드(15) -> 서머너(28)` 직후 LV1에는 소환이 없고, 다음 레벨업에서 능력 ID 22의 명령 비트 23이 설정되어 LV2부터 `소환`이 표시됩니다. 이 경로는 확인했지만 임의 클래스에 맞는 전체 능력 세트를 합성하는 규칙은 아직 확정하지 않았으므로, 현재 편집기는 클래스 선택을 능력·마법 세트 편집으로 취급하지 않습니다.
...
python3 tools/magic_flow_inventory.py
...
`docs/epilogue_probe.md`를 참고합니다. 현재 생산 checksum `ED1F` 기반
Scenario 27 진단 checksum `E93E`도 정상 전투로 베른하르트를 격파한 뒤,
일본판과 대조해 바로잡은 10개 엔딩 몽타주 레코드, 일반 선택 후일담,
크레디트와 `Fin`까지 다시 재생했습니다. 예전에 잘못 들어갔던
`리아나가 위험해`라는 1장 요약 문구는 더 이상 나오지 않으며, 엔딩 상태창의
이름·클래스 확장 글꼴도 정상입니다. 여기에 앞서는 콜드 부팅 악역 몽타주
2개도 일본 원문 33/42글자 레코드와 7글자 공유 구간을 보존해 완역했습니다.
격리 빌드 checksum `99FD`는 `DISPLAY=:104`에서 두 문장을 자동 재생한 뒤
한글화 타이틀로 정상 복귀했습니다.
...
```bash
env LD_LIBRARY_PATH=tools/blastem/lib tools/blastem/blastem "roms/builds/Langrisser II (Korean).md"
```
...
- 모든 자동화 도구는 부모 셸이 `DISPLAY=:0`이어도 기본적으로 격리 Xvfb
  `DISPLAY=:104`를 사용하고 소프트웨어 렌더러와 Xlib 캡처를 켭니다.
  실제 데스크톱은 `--desktop-display`를 명시한 경우에만 허용합니다.
  `--virtual-display :0`은 실수로 실제 화면을 선택하지 못하도록
  거부됩니다. 종료 처리도 현재 디스플레이의 BlastEm만 대상으로 하므로
  다른 화면의 인스턴스를 강제 종료하지 않습니다.
...
- checksum `18C6`은 같은 시나리오 25 수동 슬롯의 자연 활성 아론 레코드 3을 사용해 `파이터(01) -> 로드(04)` 적용을 검증합니다. 같은 ROM의 전후 GST는 `01/ID8/LV8/EXP6 -> 04/ID8/LV1/EXP0`을 기록하며 다른 아홉 플레이어 레코드는 그대로입니다. 일본판 후보 `로드/나이트/샤먼`, `클래스체인지 가능`, 적용 후 `아론/로드` 화면을 보존했습니다. 정상 클리어 뒤 SRAM 저장 지속성은 아직 별도 미검증입니다.
- checksum `1C26`은 같은 시나리오 25 수동 슬롯의 자연 활성 스코트 레코드 6을 사용해 `파이터(01) -> 호크나이트(06)` 적용을 검증합니다. 같은 ROM의 전후 GST는 `01/ID6/LV1/EXP0 -> 06/ID6/LV1/EXP0`을 기록하며 다른 아홉 플레이어 레코드는 그대로입니다. 일본판 후보 `호크나이트/나이트/로드`, `클래스체인지 가능`, 적용 후 `스코트/호크나이트` 화면을 보존했습니다. 정상 클리어 뒤 SRAM 저장 지속성은 아직 별도 미검증입니다.
...
```text
~/.local/share/blastem/Langrisser II (Korean)/save.sram
```
...
- `tools/capture_summon_application.py`는 진단 ROM에서 하인의 `소환` 명령, 8개 목록, 배치, MP 소비와 생성된 소환물 상태까지 확인합니다. checksum `C41E`의 엘리멘탈은 MP `12→7`을 소비하고 하인 런타임 그룹의 7번 구성원 슬롯에 클래스 `0x8D`, 좌표 `(12,20)`으로 생성됐으며, 하단 `엘리멘탈`과 `이동/공격/마법` 메뉴가 정상 표시됩니다. 나머지 7종도 공통 렌더러·적용 구조를 진단했습니다.
- checksum `7256`은 생산판 `1AB2`에 `서머너 LV1→LV2`의 원본 레벨업 진입점과 일본판의 전체 아이템 상점 선택기만 합성합니다. 소환 명령·목록·MP 분기와 비용표는 원본과 같습니다. 철아령을 10P에 구입해 하인에게 장비하자 자연 목록에 `엘리멘탈/형님`이 표시됐고, `형님 15/16` 적용 뒤 MP가 `16→1`, 7번 구성원 슬롯에 클래스 `0x94`, 좌표 `(14,20)`이 기록됐습니다. 이후 맵으로 잠시 돌아왔다가 긴 전용 타이틀·어트랙트 연출로 이어지는 것도 원본 동작입니다. `python3 tools/verify_natural_summon_evidence.py`가 보존 GST에서 명령 비트·장비 ID·MP·생성 클래스를 회귀 검증합니다.
...
- 게임 설정의 네 번째 행은 완료 버튼이 아니라 별도 색상·키 설정 화면을 여는 항목이므로 `설정완료`에서 `조작설정`으로 바로잡았습니다. 그 화면은 독립 glyph 목록 `0x971A2`와 `0x9AFD0` 이후 레이아웃을 사용하며, 현재 빌드는 `색상설정`, `키 설정`, `유닛검색`, `취소`, `결정/메뉴`, `설정메뉴`, `나가기`로 표시합니다. R/G/B, 숫자 0~7, 하드코딩된 A/B/C/S 접두사는 원본 glyph 슬롯 5~19를 그대로 보존해야 합니다. production `9DD0` 기반 checksum `6B21`의 `captures/run/6b21_s26_settings_menu_korean_final.png`, `captures/run/6b21_s26_control_settings_korean_final.png`에서 두 화면과 모든 고정 접두사를 확인했습니다.
...
- 첫 타이틀의 저작권 문구 아래에는 `한글화: HSP1324`를 표시합니다. 다음 줄은 일반판에서 오른쪽에 `번역:1.0.0`만 표시하고, 하드판에서는 왼쪽 모서리에 `번역:번역버전`, 오른쪽 모서리에 `하드:밸런스버전`을 분리해 표시합니다. 버전 레지스트리는 하드판 메타데이터용 통합 문자열도 유지하지만 빌더가 화면용 두 문자열로 나눕니다. 원본 타이틀은 정상 소문자 글꼴을 갖지 않고 해당 코드 일부를 지형/아이콘으로 재사용하므로 아이디는 원본 대문자/숫자 글꼴로 통일합니다. 타이틀에서만 압축 리소스 436을 VRAM 타일 `0x3A..0x73`에 덮어쓰고 다음 화면의 기존 글꼴 로더가 게임용 슬롯을 복원합니다. 같은 전용 리소스의 미사용 소문자 코드 위치를 이용해 첫 메뉴를 `새 게임 / 불러오기`로 바꾸고 버전 표시에 필요한 `번/역/밸/런/스/하/드`를 추가하며 원본 `PUSH START BUTTON`, 저작권, 게임 내 이름·클래스 글꼴은 보존합니다. 대형 로고는 사용자가 제공한 영문판 ROM의 실제 리소스 393을 사용합니다. 영문판 타입 0 원시 payload(`0x1E6E00`, 5984바이트)와 28×8 배치(`0x0A428E`, 사용 231바이트)를 해시로 검증하고, 일본판 호출 구조에 맞춰 타입 3으로 재압축해 `0x2E0000`에 배치하므로 장식체 `Langrisser`, 대형 `II`, 천사 배경이 영문판과 같습니다. checksum `2979`의 `captures/run/2979_title_translation_version.png`에서 로고·크레딧·번역 버전을 실기 확인했습니다.
...
- 전투 화면 중앙 세 번째 행은 장식이 아니라 압축 리소스 223의 일본어 `-地形-`입니다. 원본 포인터 `0x0FEB2A`, 타입 3, 해제 길이 2368바이트와 전체 해시를 검증한 뒤 `地/形` 타일 `0x47/0x48`만 `지/형`으로 바꾸고 리소스를 `0x2E2000`에 재배치합니다. 양옆 `-`는 공용 바이트 글꼴 타일 `0xB0`이므로 원본 그대로 보존합니다. Production `45D8` 기반 Scenario 4 진단 ROM `C954`의 실제 전투 `captures/run/c954_s04_terrain_battle2_04.png`에서 `엘윈/파이터`, `모건/소서러`, `-AT-`, `-DF-`, `-지형-`과 양측 수치가 동시에 정상인 것을 확인했습니다.
- 대화 초상화 리소스는 VRAM `0xA000`부터 적재되며 공용 8×8 상태 글꼴 구간도 덮습니다. 아래 Window 상태창은 이미 배치한 타일 번호를 계속 참조하므로, 파이어스 대화 중 `데몬로드`가 `로드`만 남는 문제가 발생했습니다. 원본 초상화 로더 두 곳 `0x018220/0x01840C`의 호출을 보존한 후, 실제로 덮이는 확장 구간 1..5만 큐 순서대로 복원합니다. Production `D79C` 기반 Scenario 20 진단 ROM `D2F9`에서 초상화와 `파이어스/데몬로드`가 함께 정상이며, GST의 `데/몬` 타일 `0x4AB/0x4AC`도 대화 전 바이트와 일치합니다.
- 시나리오 8 증원 그래픽은 종전 `가/스/럴` 타일을 덮어쓰므로 세 글자는 맵 상태 갱신이 복원하는 최종 확장 은행 `0x5F0..0x5F2`로 옮깁니다. 전과보고의 초기 이름 렌더러 복원만으로는 이후 캐릭터 그래픽 로드가 마지막 은행을 다시 덮었습니다. 현재는 결과 화면 설정 호출 `0x01CE40` 뒤 공통 최종 은행 로더를 실행합니다. 진단 checksum `733A`에서 `키스/레스터/스코트`를 확인했고 quickload 및 3초 대기 뒤에도 정상 표시가 유지됐습니다.
...
python3 tools/jp_text_font_analyzer.py --rom "roms/builds/Langrisser II (Korean).md" render-text --table conditions --index 0 --base 0x40000 --format jp2bpp16 --mapped --cols 18 --scale 4
python3 tools/jp_text_font_analyzer.py render-direct-strings --start 0x97000 --end 0x97800 --scale 3
python3 tools/jp_text_font_analyzer.py --rom "roms/builds/Langrisser II (Korean).md" render-pointer-text --pointer-table 0xA1D7C --low 0xA1E10 --high 0xA2C00 --glyph-list 0x1E9000 --cols 15 --scale 3 --out item_descriptions_korean_probe_cols15.png
