...
# Loren's editor design uses a two-step crimson ramp, distinct from the Scenario
# 1 militia's ivory armor. The live map cannot add per-class CRAM colours, so
# reuse its bright-red/deep-red pair for a visible red gradient instead of a
# flat tint. Preserve the stock shield, trim, and blade pixels.
LOREN_SPRITE_COLOR_INDEX_REMAP = {
    0x1: 0xB,  # gray armor face -> bright red
    0xE: 0xC,  # dark gray armor shade -> deep red
}
...
SCENARIO1_EVENT_PAGE_PATCHES = {
    ...
    0x185E50: (0x185E7E, "그럼 지금부터 발드님의\n퇴로를 확보합니다!"),
    0x185E80: (0x185EA4, "이번 임무가 처음이지?"),
...
SCENARIO_TEXT_OVERRIDES = {
    ...
    10: (
        ...
        "제국의 참모이자 흑룡마도사단을 이끄는 에그베르트가 그때를 노려 몰래 "
        "함정을 파 두었다."
    ),
    25: (
        "시나리오 26\n"
...
def patch_title_logo_resource(
    ...
    *,
    ...
    if original_pointer != TITLE_LOGO_RESOURCE_ORIGINAL_POINTER:
        raise ValueError(
            f"title logo resource pointer changed: 0x{original_pointer:06X}"
        )
    if extended_pointer != TITLE_LOGO_RESOURCE_ORIGINAL_POINTER:
        raise ValueError(
            f"extended title logo resource pointer changed: 0x{extended_pointer:06X}"
        )
    if data[original_pointer] != 0x03:
        raise ValueError("title logo is not a type 3 compressed resource")
    source = decompress_9dfe(data, original_pointer + 1)
    if len(source) != TITLE_LOGO_RESOURCE_ORIGINAL_SIZE:
        raise ValueError("title logo decoded size changed")
    ...
    if resource_end > TITLE_LOGO_RESOURCE_RELOC_LIMIT:
        raise ValueError("localized title logo exceeds its reserved bank")
    if any(
        value != 0xFF
        for value in data[TITLE_LOGO_RESOURCE_RELOC_BASE:resource_end]
    ):
        raise ValueError("localized title logo bank is not blank")
